   ...
   //}
	...
	//encodage utf8
		...
		for($y=$_REQUEST['annee_date_deb']; $y<=$_REQUEST['annee_date_fin'] ; ++$y ){
			...
			//affichage des mois
			for($m=$mois_deb; $m<=$mois_max ; ++$m ){
				$row->addCell( new odsTableCellString(utf8_encode($this->get_qte_cmd_mensuel($ref_art,$y,$m))));
				//sous total par article
				$qtesoustotal+= $this->get_qte_cmd_mensuel($ref_art,$y,$m);  
			}
		}
	$row->addCell( new odsTableCellString(utf8_encode($qtesoustotal), $fond_gris));
	$table->addRow($row);
	}
	$row   = new odsTableRow();
	...
	for($y=$_REQUEST['annee_date_deb']; $y<=$_REQUEST['annee_date_fin'] ; ++$y ){
		...
		//affichage des mois
			for($m=$mois_deb; $m<=$mois_max ; ++$m ){
				...
				$row->addCell( new odsTableCellString(utf8_encode($qte), $fond_gris));
				$qte_total += $qte;
			}
	}
	$row->addCell( new odsTableCellString(utf8_encode($qte_total), $fond_gris));
	$table->addRow($row);
