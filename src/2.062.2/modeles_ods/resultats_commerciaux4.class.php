...
class ods_resultats_commerciaux4{
	var $code_ods_modele = "resultats_commerciaux4";
	var $ods;
	var $table;
	var $commerciaux;				
...
public function ods_resultats_commerciaux4 () {
...
//	return $this;
	...
	$this->create_stylecol('3.5 cm');
	$this->create_stylecol('5 cm');
	$this->create_stylecol('3 cm');
	$this->create_stylecol('1.8 cm');
	$this->create_stylecol('2 cm');
	$this->create_stylecol('2 cm');
	$this->create_stylecol('1.5 cm');
	$this->create_stylecol('2.3 cm');
	...
	switch ($commercial->doc_fom_comm) {
		...
		$this->create_entete();
		//Liste des factures par commercial
		foreach ($commercial->docs as $docu) {
			// Liste article par facture
			foreach($docu->detail as $article){
				$tmp_article = new article ($article->ref_article);
				...
				$ca_article = (($article->pu_ht * $article->qte)-($article->pu_ht * $article->qte * $article->remise / 100))*($docu->part/100);
				$marge_article = ((($article->pu_ht *$article->qte)-($article->pu_ht * $article->qte * $article->remise / 100))-($pa_article * $article->qte))*($docu->part/100);
				...
				preg_match('#([0-9\.]*)%CA\+([0-9\.]*)%Mg#i', $comm->__toString(), $result);
	
				$row=$this->create_row();
				$this->create_cell(date_Us_to_Fr($docu->date_creation_doc),$row);				
				$this->create_cell($docu->ref_doc,$row);
				$this->create_cell($docu->nom,$row);
				$this->create_cell((isset($article->ref_interne) ? $article->ref_interne:$article->ref_article),$row);
				$this->create_cellend($article->qte,$row);
				$this->create_celleuro($article->pu_ht,$row);
				$this->create_celleuro($ca_article,$row);
				$this->create_cellend($result[1],$row);
				$this->create_celleuro($comm->calcul_commission ($ca_article, $marge_article),$row);
				}
		}
	//Ajout de la table
	$this->ods->addTable($this->table);
	...
	// Download the file
	$this->ods->downloadOdsFile("Résultat commerciaux par article.ods");
}
...
public function create_entete() {
	...
	$this->create_cellgrey('Article',$row);
	$this->create_cellgrey('Quantité',$row);
	$this->create_cellgrey('PU HT',$row);
	$this->create_cellgrey('CA HT',$row);
	$this->create_cellgrey('Taux %',$row);
