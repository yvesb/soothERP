...
function document_recherche_article() {
	var AppelAjax = new Ajax.Updater(
		...
		encoding:     'UTF-8',
		parameters: { recherche: '1', ref_art_categ : $F('ref_art_categ_s'), lib_article: escape($F('lib_article_s')), page_to_show: $F('article_page_to_show_s'), ref_constructeur: $F('ref_constructeur_s'), orderby: $F('article_orderby_s'), orderorder: $F('article_orderorder_s'), ref_doc: $F('ref_doc'), recherche_auto: $F('article_recherche_auto'), from_rapide_search: from_rapide_search},
		evalScripts:true, 
...
function caisse_recherche_client_simple() {
	var AppelAjax = new Ajax.Updater(
		...
		contentType:  'application/x-www-form-urlencoded',
		encoding:     'UTF-8',
		parameters: { recherche: '1', nom : escape($F('nom_s')), page_to_show: $F('page_to_show_s'), orderby: $F('orderby_s'), orderorder: $F('orderorder_s')},
		evalScripts:true, 
		onLoading:S_loading, onException: function () {S_failure();}, 
		onComplete:H_loading}
		);
...
function caisse_ajouter_article(ref_ticket, ref_contact, ref_article) {
		...
		var AppelAjax = new Ajax.Request(
			...
			{
				parameters: {ref_ticket: ref_ticket, ref_contact: ref_contact, ref_article : ref_article},
				evalScripts:true,
				onLoading:S_loading,
				onSuccess: function (requester){
				requester.responseText.evalScripts();
...
function caisse_suppr_article(ref_ticket, ref_ligne) {
		var AppelAjax = new Ajax.Request(
			...
			{
				parameters: {ref_ticket: ref_ticket, ref_ligne: ref_ligne},
				evalScripts:true,
				onLoading:S_loading,
				onSuccess: function (requester){
				requester.responseText.evalScripts();
...
function caisse_maj_ligne_ticket(ref_ticket, ref_ligne, remise, puttc, qte ) {
		var AppelAjax = new Ajax.Request(
			...
			{
				parameters: {ref_ticket: ref_ticket, ref_ligne: ref_ligne, remise : remise, puttc : puttc, qte : qte},
				evalScripts:true,
				onLoading:S_loading,
				onSuccess: function (requester){
				requester.responseText.evalScripts();
...
function caisse_charger_ticket (ref_ticket) {
	var AppelAjax = new Ajax.Request(
		...
		{
		parameters: {ref_ticket: ref_ticket},
		evalScripts:true,
		onLoading:S_loading,
		onSuccess: function (requester){
		requester.responseText.evalScripts();
...
function caisse_encaisser_ticket (ref_ticket, moyens_de_paiememnt, montants,  type_print) {
	var AppelAjax = new Ajax.Updater(
		...
		{
		parameters: {ref_ticket: ref_ticket, moyens_de_paiememnt : moyens_de_paiememnt, montants: montants, type_print : type_print},
		evalScripts:true,
		onLoading:S_loading,
		onSuccess: function (requester){
		requester.responseText.evalScripts();
...
function caisse_ajouter_moyen_de_paiement (ref_ticket, moyen_de_paiement) {
	var AppelAjax = new Ajax.Updater(
		...
		{
		parameters: {ref_ticket: ref_ticket, moyen_de_paiement : moyen_de_paiement},
		evalScripts:true,
		onLoading:S_loading,
		onSuccess: function (requester){
		requester.responseText.evalScripts();
		...
		case "recherche_article" :{
			...
			var AppelAjax = new Ajax.Updater(
					...
					"caisse_panneau_recherche_article.php",
					{
						...
						contentType	:	'application/x-www-form-urlencoded',
						encoding	:	'UTF-8',
						parameters	:	{select_racine_art_categs: select_racine_art_categs},
						evalScripts	:	true,
						onLoading	:	S_loading,
...
function maj_etat_ticket (ref_doc, new_etat_doc, fonction_called_after_maj_etat_ticket) {
	var AppelAjax = new Ajax.Updater(
		...
		{
		parameters: {ref_doc: ref_doc, new_etat_doc: new_etat_doc },
		evalScripts:true,
		onLoading:S_loading,
		onSuccess: function (requester){
		requester.responseText.evalScripts();
...
function script_called_after_maj_etat_60_from_afficher_ticket(rafraichir_accueil){
	...
	else{	change_panneau_bas("tickets_en_attente");}
	H_loading();
}
...
function ajouterArticleTicket(suffixe, monaie, cell_LIB, cell_QTE, cell_PUTTC, cell_REMISE, cell_PRIXTTC){
	...
	Event.observe(suffixe, "click", function(evt){
		Event.stop(evt);
		if(ticket_is_unlock){ caisse_select_line(suffixe); }
	}, false);
}
