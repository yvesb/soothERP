	...
	<div class="reduce_in_edit_mode">
		<table class="minimizetable">
			...
			if(empty($liste_categories_fournisseur))  {
				...
				<td>
				<input type="checkbox" value="0" id="tous_fourn" name="tous_fourn" <?php if (!count($fourn_cat) || !$fourn_cat[0]) {?>checked="checked"<?php } ?> >Toutes 
		...
		<tr>
				...
				foreach ($liste_categories_fournisseur as $cat_fourn) {
				?>
				<input type="checkbox" name="fourn_cat_<?php echo $cat_fourn->id_fournisseur_categ;?>" id="fourn_cat_<?php echo $cat_fourn->id_fournisseur_categ;?>" value="<?php echo $cat_fourn->id_fournisseur_categ;?>" <?php if(array_search($cat_fourn->id_fournisseur_categ, $fourn_cat)!== false) { ?> checked="checked" <?php } ?>/><?php echo ($cat_fourn->lib_fournisseur_categ);?>
