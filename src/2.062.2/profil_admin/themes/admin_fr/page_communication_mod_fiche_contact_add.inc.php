...
var erreur=false;
var choisir_id_type_contact = false;
...
foreach ($_ALERTES as $alerte => $value) {
  if ($alerte=="choisir_id_type_contact") {
    echo "choisir_id_type_contact=true;";
    echo "erreur=true;\n";
  }
...
if (erreur) {
  ...
  if (choisir_id_type_article) {
    window.parent.document.getElementById("choix_type_contact").className="alerteform_xsize";
    texte_erreur += "Vous devez sélectionner un type de document.<br/>";
  } else {
    window.parent.document.getElementById("choix_type_contact").className="classinput_xsize";
  }
  if (choisir_source) {
  ...
  if (choisir_id_pdf_modele) {
    window.parent.document.getElementById("choix_id_pdf_contact").className="alerteform_xsize";
    texte_erreur += "Vous devez sélectionner un modèle de document source.<br/>";
  } else {
    window.parent.document.getElementById("choix_id_pdf_contact").className="classinput_xsize";
  }
...
window.parent.page.traitecontent('Communication_mod_fiche_contact','communication_mod_fiche_contact.php','true','sub_content');
