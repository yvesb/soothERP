...
if (isset($_REQUEST['ip'])) {
	$form['ip'] = trim(urldecode($_REQUEST['ip']));
	$search['ip'] = trim(urldecode($_REQUEST['ip']));
}
$form['id_profil'] = 0;
...
if (isset($_REQUEST['recherche'])) {
	...
	if ($search['nom']) {
		...
		for ($i=0; $i<count($libs); $i++) {
			$lib = trim($libs[$i]);
			$query_where 	.= " nom LIKE '%".addslashes($lib)."%' ||  ul.ref_user LIKE '%".addslashes($lib)."%'  ||  u.pseudo LIKE '%".addslashes($lib)."%' ||  a.ref_contact LIKE '%".addslashes($lib)."%'  "; 
	...
	if ($search['ip']) { 
		if ($query_where) { $query_where .= " && "; }
		$query_where 	.= " ul.ip LIKE '%".$search['ip']."%' ";
	}
	// Profils
	if ($search['id_profil']) { 
...
include ($DIR.$_SESSION['theme']->getDir_theme()."page_utilisateur_histo_result.inc.php");
