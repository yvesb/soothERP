...
if (!count($GLOBALS['_ALERTES'])) {
  ...
  if (isset($class_name) && isset($config_name) && ($class_name == $config_name) && !count($GLOBALS['_ALERTES'])) {
    ...
    $query = "INSERT INTO stats_modeles_pdf (id_pdf_modele, id_stat, `usage`)
              VALUES ('".$id_pdf_modele."', '1', 'inactif' ) ";
    $bdd->exec ($query);
...
include ($DIR.$_SESSION['theme']->getDir_theme()."page_communication_mod_fiche_stats_add.inc.php");
?>  