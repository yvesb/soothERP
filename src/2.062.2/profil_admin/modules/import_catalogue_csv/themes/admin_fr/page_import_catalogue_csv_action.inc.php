...
<?php 
		...
		switch ($_REQUEST["fonction_generer"]) {
			...
			case "import":
				?>
				window.parent.alerte.alerte_erreur ('Import éffectué', "<?php echo $GLOBALS['_INFOS']['count_import']."/".count($liste_rec); ?> enregistrements importés",'<input type="submit" id="bouton0" name="bouton0" value="Ok" />', function () {window.parent.page.verify('retour_import','modules/import_catalogue_csv/retour_import.php','true','formFrame');});
				window.parent.page.verify('import_catalogue_csv_step2','modules/import_catalogue_csv/import_catalogue_csv_step2.php','true','sub_content');
				<?php
			break;
