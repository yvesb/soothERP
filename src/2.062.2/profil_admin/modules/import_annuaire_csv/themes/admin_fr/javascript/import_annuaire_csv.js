...
function annuaire_import() {
		var AppelAjax = new Ajax.Updater(
									"resultat_imports", 
									"modules/import_annuaire_csv/import_annuaire_csv_liste_result.php", {
									method: 'post',
									asynchronous: true,
...
function action_annuaire_import(action_selection, second_id , length_list) {
		...
		if (action_selection == "import") {
				var AppelAjax = new Ajax.Request(
													"modules/import_annuaire_csv/import_annuaire_csv_action.php?fonction_generer="+action_selection+liste_doc, 
													{
													evalScripts: true, 
													onLoading: S_loading,
													parameters: { recherche: '1', id_profil: $F('id_profil_s'), page_to_show: $F('page_to_show_s'), orderby: $F('orderby_s'), orderorder: $F('orderorder_s')},
...
function annuaire_import_maj_limite(limite) {
		var AppelAjax = new Ajax.Request(
									"modules/import_annuaire_csv/import_annuaire_csv_maj_limite.php", {
									method: 'post',
									asynchronous: true,
									contentType:  'application/x-www-form-urlencoded',
									encoding:     'UTF-8',
...
function supprime_import_ligne (ligne) {
	...
	Event.observe("bouton1", "click", function () {
		var AppelAjax = new Ajax.Request(
											"modules/import_annuaire_csv/import_annuaire_csv_action.php?fonction_generer=supprimer&id_rec"+ligne+"="+ligne, 
											{
											evalScripts: true, 
											parameters: { recherche: '1', id_profil: $F('id_profil_s'), page_to_show: $F('page_to_show_s'), orderby: $F('orderby_s'), orderorder: $F('orderorder_s')},
											onLoading: S_loading,
