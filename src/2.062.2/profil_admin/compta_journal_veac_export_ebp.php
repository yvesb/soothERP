...
// *************************************************************************************************************
	...
	header('Content-Type: text/plain; name="ECRITURES'.$complement_nom_fichier.'.txt"');
	header('Content-Disposition: attachment; filename=ECRITURES'.$complement_nom_fichier.'.txt;'); 
	$i = 1;
	...
	foreach ($fiches as $fiche) {
		...
		echo $i.",".date("dmY",strtotime($fiche->date_creation_doc)).",".$journal_exp.",".$fiche->numero_compte.",".$type_fact.",\"".substr($fiche->lib_compte,0,40)."\",\"".substr($fiche->ref_doc,0,3).substr($fiche->ref_doc,11,16)."\",".abs(number_format($fiche->montant, $TARIFS_NB_DECIMALES, ".", ""	)).",".$DorC.",".date("dmY",strtotime($fiche->date_echeance)).",".$MONNAIE[2]."\n";
