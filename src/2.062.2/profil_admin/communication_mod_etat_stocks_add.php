...
if (!count($GLOBALS['_ALERTES'])) {
  ...
  if (isset($class_name) && isset($config_name) && ($class_name == $config_name) && !count($GLOBALS['_ALERTES'])) {
    ...
    $query = "INSERT INTO pdf_modeles (id_pdf_type, lib_modele, desc_modele , code_pdf_modele)
              VALUES ('6', '".addslashes($_REQUEST["lib_modele"])."', '".addslashes($_REQUEST["desc_modele"])."',
                      '".$class_name."' ) ";
    $bdd->exec ($query);
    $id_pdf_modele= $bdd->lastInsertId();
    $query = "INSERT INTO stocks_modeles_pdf (id_pdf_modele, `usage`)
              VALUES ('".$id_pdf_modele."', 'inactif' ) ";
    $bdd->exec ($query);
...
include ($DIR.$_SESSION['theme']->getDir_theme()."page_communication_mod_etat_stocks_add.inc.php");
?>  