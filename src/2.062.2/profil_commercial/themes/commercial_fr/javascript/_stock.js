...
function stock_etat_imprimer () {
		...
		if ($F('id_stock_l').length >1 || $F('id_stock_l').length == 0 || ($F('id_stock_l').length == 1 && $F('id_stock_l')== "")) {
			...
			if ($F('id_stock_l').length >0) {
				...
				window.open('stocks_etat_imprimer.php?ref_art_categ='+$F('ref_art_categ_s')+'&ref_constructeur='+$F('ref_constructeur_s')+'&aff_pa='+f_aff_pa_s+'&orderby='+$F('orderby_s')+'&orderorder='+$F('orderorder_s')+'&id_stock='+id_stock_s+'&in_stock='+$F("in_stock_s")+'', '_blank');
			}
		}
}
