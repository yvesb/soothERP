...
class pdf_content_doc_cdc_standard extends pdf_content_doc_standard{
	...
	protected function create_pdf_pieds () {
		...
		if(!isset($this->AFF_PRIX) || $this->AFF_PRIX){
			...
			$this->pdf->SetXY($this->MARGE_GAUCHE + $largeur_bloc_tva, $this->PIEDS_HAUTEUR_DEPART+($decalage*3)+7);
			...
			foreach($reglements as $reglement) 
			{
				...
				else if($reglement->type_reglement=="EcheanceResume")
				{
					...
					$this->pdf->Cell ($this->LARGEUR_TOTALE_CORPS-$largeur_bloc_montant-$largeur_bloc_tva, 3, $chaine, '0', 1, 'L');
				}
		...
		$this->pdf->SetXY($this->MARGE_GAUCHE + $largeur_bloc_tva, $this->PIEDS_HAUTEUR_DEPART+27);
		$this->pdf->SetFont('Arial', '', 7);
		$this->pdf->Cell ($this->LARGEUR_TOTALE_CORPS-$largeur_bloc_montant-$largeur_bloc_tva, 5,"Mention manuscrite « Bon pour accord » + Tampon & Signature" , '0', 0, 'L');
		$this->pdf->SetXY($this->MARGE_GAUCHE + $largeur_bloc_tva, $this->PIEDS_HAUTEUR_DEPART+27);
		$this->pdf->Cell ($this->LARGEUR_TOTALE_CORPS-$largeur_bloc_montant-$largeur_bloc_tva, 5," " , '1', 0, 'L');
