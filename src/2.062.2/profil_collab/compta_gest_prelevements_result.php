...
$where = array('1');
if(isset($_REQUEST["categ_client"]) && $_REQUEST["categ_client"] && $_REQUEST["categ_client"] != "all")
...
$where[] = "cba.id_reglement_mode IN (6)";
...
if(isset($_REQUEST["num_compte"]) && $_REQUEST["num_compte"])
    $where[] = "cb.numero_compte LIKE '%".$_REQUEST["num_compte"]."%'";
$where[] = "cba.id_reglement_mode IN (5)";
...
//print_r($query); exit;
$resultat = $bdd->query ($query);
$results_traitena = array();
while ($tmp = $resultat->fetchObject()) { $results_traitena[] = $tmp; }

// *************************************************************************************************************
// AFFICHAGE
// *************************************************************************************************************

include ($DIR.$_SESSION['theme']->getDir_theme()."page_compta_gest_prelevements_result.inc.php");
