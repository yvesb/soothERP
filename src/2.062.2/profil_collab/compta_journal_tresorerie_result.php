 ...
 $queryd = "SELECT cjo.id_operation ,cjo.id_journal, cjo.numero_compte, cjo.montant, cjo.ref_operation, cjo.date_operation, cjo.id_operation_type,
	...
	while ($fiche = $resultatd->fetchObject()) {
		...
		switch ($fiche->id_operation_type) {
			// transfert de fond caisse vers banque
			case 1:
				...
				if ($inf_tmp = $resultat_tmp->fetchObject()) {
					...
					$fiche->libelle = "Remise bancaire vers banque ".$inf_tmp->lib_compte;
				}
			break;
			...
			case 2:
				...
				if ($inf_tmp = $resultat_tmp->fetchObject()) {
					$fiche->compte_tier = "";
					$fiche->id_compte_caisse = $inf_tmp->id_compte_caisse_source;
					$fiche->libelle = "Remise bancaire depuis caisse ".$inf_tmp->lib_caisse;
				}
			break;
			...
			case 3:
				...
				if ($inf_tmp = $resultat_tmp->fetchObject()) {
					...
					$fiche->compte_tier = "";
					$fiche->libelle = "Retrait bancaire vers caisse ".$inf_tmp->lib_caisse;
				}
			break;
			// transfert de fond caisse depuis banque
			case 4:
 				$query_tmp = "SELECT ccr.id_compte_caisse_retrait, ccr.id_compte_caisse_destination, ccr.id_compte_bancaire_source, 
											cc.lib_caisse ,
											cc2.lib_compte, cc2.numero_compte
			...
			case 9:
 				$query_tmp = "SELECT cct.id_compte_caisse_transfert, cct.id_compte_caisse_source, cct.id_compte_caisse_destination, 
											...
											WHERE id_compte_caisse_transfert = '".$fiche->ref_operation."' 
											";
				$resultat_tmp = $bdd->query($query_tmp);
				if ($inf_tmp = $resultat_tmp->fetchObject()) {
					$fiche->id_compte_caisse = $inf_tmp->id_compte_caisse_source;
					$fiche->compte_tier = "";
					$fiche->libelle = "Transfert vers ".$inf_tmp->lib_caisse_dest;
				}
			break;
			// transfert de fond caisse vers caisse
			case 10:
				...
				if ($inf_tmp = $resultat_tmp->fetchObject()) {
					$fiche->id_compte_caisse = $inf_tmp->id_compte_caisse_source;
					$fiche->compte_tier = "";
					$fiche->libelle = "Transfert depuis ".$inf_tmp->lib_caisse_source;
				}
			break;
		}
...
if (isset($_REQUEST["print"])) {
	...
	$code_pdf_modele = "journaux_tresorerie";
	$GLOBALS['PDF_OPTIONS']['HideToolbar'] = 0;
	$GLOBALS['PDF_OPTIONS']['AutoPrint'] = 1;
	
	$journal = new compta_journaux($form['id_journal']);
