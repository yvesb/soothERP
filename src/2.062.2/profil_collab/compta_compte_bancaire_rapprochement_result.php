...
$form['arapp'] = $search['arapp'] = 0;
if (isset($_REQUEST['arapp'])) {
	$form['arapp'] = $_REQUEST['arapp'];
	$search['arapp'] = $_REQUEST['arapp'];
}
$form['montants'] = $search['montants'] = "";
if (isset($_REQUEST['montants'])) {
	$form['montants'] = $_REQUEST['montants'];
	$search['montants'] = $_REQUEST['montants'];
}
...
if (isset($_REQUEST['recherche'])) {
	...
	if ($search['arapp']) {
		$query_where 	.= " && ISNULL(cbm.id_operation) ";
	}
	if ($search['montants'] == "deb") {
...
include ($DIR.$_SESSION['theme']->getDir_theme()."page_compta_compte_bancaire_rapprochement_result.inc.php");
