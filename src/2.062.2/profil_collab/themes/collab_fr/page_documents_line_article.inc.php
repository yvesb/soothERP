		...
		<td style="width:280px; padding-left:3px">
			  ...
			  if (is_array($contenu->sn)) {
					...
					if ($contenu->gestion_sn == 2)  {
						...
						foreach ($nb_nl as $nl_key=>$nl_val) {
							...
							<input value="<?php if (isset($nl_key)) {echo $nl_key;}?>" type="text" id="art_nl_<?php echo $indentation_contenu;?>_<?php echo $i;?>" name="art_nl_<?php echo $indentation_contenu?>_<?php echo $i;?>"  size="17"/>
			...
			if ($contenu->modele == "service_abo"){
		      ...
		      <span>Date de d&eacute;but&nbsp;:&nbsp;</span>
		      	<input value="<?php if(isset($duree_abo->date_debut)){echo date('d/m/Y', strtotime($duree_abo->date_debut));} ?>" id="date_deb_abo_<?php echo $indentation_contenu?>" name="date_deb_abo_<?php echo $indentation_contenu?>" type="text" style="width:67px;" class="classinput_xsize"/>
			  </div>
			  <div>
