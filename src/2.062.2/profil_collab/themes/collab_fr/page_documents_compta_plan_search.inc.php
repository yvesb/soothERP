	...
	<table style="width:97%">
		...
		<tr>
			...
			Event.observe("search_compte_kw", "click",  function(evt){
				Event.stop(evt);
				load_result_plan_compte_recherche("kw", $("cpe_search_lib").value,  "documents_compta_plan_search_result.php");
			}, false);
		...
		<tr>
			...
			Event.observe("search_compte_num", "click",  function(evt){
				Event.stop(evt);
				load_result_plan_compte_recherche("num", $("cpe_search_num").value,  "documents_compta_plan_search_result.php");
			}, false);
		...
		<tr>
			...
			Event.observe("search_compte_fav", "click",  function(evt){
				Event.stop(evt);
				load_result_plan_compte_recherche("fav", "1",  "documents_compta_plan_search_result.php");
			}, false);
			...
			<td colspan="2">
				...
				Event.observe("valider", "click",  function(evt){
					...
					if ($("retour_value").value != "") {
						$("numero_compte_<?php if (isset($_REQUEST["indent"])) { echo $_REQUEST["indent"];}?>").value = $("retour_value").value;
						$("numero_compte_compta_<?php if (isset($_REQUEST["indent"])) { echo $_REQUEST["indent"];}?>").innerHTML = $("retour_value").value;
					} else {
						$("numero_compte_<?php if (isset($_REQUEST["indent"])) { echo $_REQUEST["indent"];}?>").value = "";
						$("numero_compte_compta_<?php if (isset($_REQUEST["indent"])) { echo $_REQUEST["indent"];}?>").innerHTML = "<?php if(isset($_REQUEST["num_compte"])) {echo $_REQUEST["num_compte"];} else {echo "...";}?>";
					}
					$("lib_compte_<?php if (isset($_REQUEST["indent"])) { echo $_REQUEST["indent"];}?>").innerHTML = $("retour_lib").value;
...
function send_if_Key_RETURN (evt, type) {
	...
	case Event.KEY_RETURN:     
			Event.stop(evt);
			load_result_plan_compte_recherche(type, field_value,  "documents_compta_plan_search_result.php");
	break;   
	}
}
