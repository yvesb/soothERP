	...
	<?php if(count($fiches)>0) { ?>
		...
		$class_colorise= ($colorise % 2)? 'colorise1' : 'colorise2'; ?>
			<tr class="<?php  echo  $class_colorise; ?>" id="line_<?php echo $colorise; ?>">
				...
				<td style="text-align:right;">
					<?php 
					$total_qte_A_livre += $fiche->qte_a_livrer;
					echo $fiche->qte_a_livrer; ?>
				</td>
				<td>&nbsp;</td>
			</tr>
		<?php } 
		...
		<script type="text/javascript">
			Event.observe("export_liste_format_CSV", "click",  function(evt){
				Event.stop(evt);
				page.catalogue_articles_export_CSV_blc_pret_au_depart("<?php echo $article->getRef_article(); ?>");
				//page.traitecontent("resultat","catalogue_articles_export_CSV_blc_pret_au_depart.php?ref_article=<?php echo $article->getRef_article(); ?>", true ,"resultat");
			}, false);
		</script>
		<div id="resultat"></div>
		<br/>
