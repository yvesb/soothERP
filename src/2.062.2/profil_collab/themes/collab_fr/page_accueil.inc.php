...
//				if($open_doc->id_type_doc == 11 && !$_SESSION['user']->check_permission ("21")){continue;}
				...
				<tr id="open_doc_<?php echo $open_doc->ref_doc;?>" style="cursor:pointer">
					...
					<?php	if ($open_doc->montant_ttc) { echo number_format($open_doc->montant_ttc, $TARIFS_NB_DECIMALES, ".", ""	)." ".$MONNAIE[1]; }?>					</td>
					<td>
...
<!--	 -->
		...
		<div class="titre_accueil_fav">
		Mes Liens Favoris
		</div>
		<div id="fav_content"> <!--style="height:220px; OVERFLOW-Y: auto; OVERFLOW-X: auto;"-->
		<?php
		foreach ($liste_links as $link) {
			?>
			<a href="<?php if (!preg_match("#^((http|https|ftp)://)#", $link->url_web_link)) {echo "http://";} ?><?php echo $link->url_web_link;?>" target="_blank" class="link_fav">
			<table width="100%" >
			<tr>
