...
/*$string =  '<script language="JavaScript">alert(\'';
...
if (!isset($_REQUEST["id_type_article"]) || !$_REQUEST["id_type_article"]) {$GLOBALS['_ALERTES']['choisir_id_type_article'] = 1;}
...
if (!count($GLOBALS['_ALERTES'])) {
	...
	if (isset($class_name) && isset($config_name) && ($class_name == $config_name) && !count($GLOBALS['_ALERTES'])) {
		$query = "INSERT INTO pdf_modeles (id_pdf_type, lib_modele, desc_modele , code_pdf_modele)
							VALUES ('3', '".addslashes($_REQUEST["lib_modele"])."', '".addslashes($_REQUEST["desc_modele"])."',
											'".$class_name."' ) ";
		$bdd->exec ($query);
		$id_pdf_modele= $bdd->lastInsertId();
		$query = "INSERT INTO art_categs_modeles_pdf (id_pdf_modele, ref_art_categ, `usage`)
							VALUES ('".$id_pdf_modele."', '".$_REQUEST["id_type_article"]."', 'inactif' ) ";
		$bdd->exec ($query);
...
include ($DIR.$_SESSION['theme']->getDir_theme()."page_communication_mod_fiche_art_add.inc.php");
?>	