<?php
// *************************************************************************************************************
// [ADMINISTRATEUR] RECHERCHE D'UNE FICHE DE CONTACT COLLAB
// *************************************************************************************************************
...
$form['id_fonction'] = 0;
if (isset($_REQUEST['id_fonction']) && $_REQUEST['id_fonction']) {
	$form['id_fonction'] = $_REQUEST['id_fonction'];
	$search['id_fonction'] = $_REQUEST['id_fonction'];
}
...
if (isset($_REQUEST['recherche'])) {
	...
	// Profils
	if (isset($search['id_fonction'])) { 
		$query_join 	.= " LEFT JOIN annu_collab_fonctions acf ON acf.ref_contact = a.ref_contact "; 
		$query_join 	.= " LEFT JOIN fonctions f ON f.id_fonction = acf.id_fonction "; 
		if ($query_where) { $query_where .= " && "; }
		$query_where 	.= "f.id_fonction = '".$search['id_fonction']."'";
	}
	...
	$query = "SELECT a.ref_contact, nom, lib_civ_court,
									 ...
									 tel1, tel2, fax, email, co.ordre, date_archivage,
									 url, si.ordre
						FROM annuaire a 
...
include ($DIR.$_SESSION['theme']->getDir_theme()."page_annuaire_gestion_collabs_liste_result.inc.php");
