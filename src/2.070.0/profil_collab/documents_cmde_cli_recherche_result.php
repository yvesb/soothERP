...
if (isset($_REQUEST['orderby'])) {
    if ((!($_REQUEST['orderby']== "date livraison")) || (empty ($_REQUEST['cmdeaprep']))){
...
if (isset($_REQUEST['recherche'])) {
	...
	$cpteur=0;
         if (!empty ($sas) && !empty($arts) && !empty($tabalivrer)){
	...
	$query = "SELECT d.ref_doc, d.id_type_doc, dt.lib_type_doc, d.id_etat_doc, de.lib_etat_doc, ref_contact, nom_contact, dc.id_stock,
        ...
        $query_a_livrer = "SELECT d.ref_doc, d.id_type_doc, dt.lib_type_doc, d.id_etat_doc, de.lib_etat_doc, ref_contact, nom_contact, dc.id_stock,
        ...
        if ($search['cmdeaprep']){
        $resultat = $bdd->query($query_a_livrer);
        }else{
	...
	$query = "SELECT dl.ref_doc_line, dl.ref_doc, dl.ref_article, dl.lib_article, dl.desc_article, dl.qte, dl.pu_ht, dlc.qte_livree,
...
include ($DIR.$_SESSION['theme']->getDir_theme()."page_documents_cmde_cli_recherche_result.inc.php");
?>