		...
		foreach ($_REQUEST as $variable => $valeur) {
			...
			if ((substr ($variable, 0, 14) == "CHK_EXIST_CHQ_")  ) {
				if (isset($_REQUEST[str_replace("CHK_", "", $variable)])) {
					$info["CHQ"]["liste_cheques"][] = array("montant_depot"=>$_REQUEST[str_replace("CHK_", "", $variable)]."", "infos_depot"=>$_REQUEST["REF_".str_replace("CHK_", "", $variable)].";".$_REQUEST["NUM_".str_replace("CHK_", "", $variable)].";".$_REQUEST["BNQ_".str_replace("CHK_", "", $variable)].";".$_REQUEST["POR_".str_replace("CHK_", "", $variable)]);
				}
			}
...
include ($DIR.$_SESSION['theme']->getDir_theme()."page_compta_depot_bancaire_caisse_create.inc.php");
