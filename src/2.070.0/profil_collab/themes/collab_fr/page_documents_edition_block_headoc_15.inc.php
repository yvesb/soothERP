		...
		<td colspan="3">
						...
						<table cellpadding="0" cellspacing="0" border="0" style="width:100%;">
							<tr style=" line-height:24px; height:24px;">
								...
								<td style="width:250px; font-weight:bolder;">
									<a id="head_date_creation" class="modif_input2">
										<?php if ($document->getDate_creation ()!= 0000-00-00) { echo  date_Us_to_Fr ($document->getDate_creation()); } ?>
										&nbsp;
									</a>
									<input type="text" value="<?php if ($document->getDate_creation()!=0000-00-00) { echo date_Us_to_Fr ($document->getDate_creation ()); }?>" 
										id="date_creation" name="date_creation" style="display:none" />
									<input type="hidden" value="<?php if ($document->getDate_creation ()!=0000-00-00) { echo date_Us_to_Fr ($document->getDate_creation ()); }?>" 
							...
							<tr style=" line-height:24px; height:24px;">
								...
								<td style="width:250px;">
									<a id="head_code_affaire" class="modif_input2">
									<?php echo $document->getCode_affaire(); ?>&nbsp;</a>
								...
								<td style="width:20%; text-align:center;">
									<a href="#documents_recherche.php?code_affaire=<?php echo $document->getCode_affaire();?>" target="_blank" id="lien_recherche_avancee_code_affaire"
										<?php if($document->getCode_affaire () == ""){ ?>style="display: none;"<?php } ?>>
										VOIR
									</a>	
								</td>
							</tr>
						<?php if (count($_SESSION['magasins'])>1) { ?>
							<tr style=" line-height:24px; height:24px;">
								...
								<td style="width:250px;">
									...
									foreach ($magasins_supp as $magasin_supp) {
										if (!$magasin_supp->actif && $magasin_supp->id_magasin == $document->getId_magasin ()) { ?>
											<option value="<?php echo $magasin_supp->id_magasin; ?>" selected="selected" >
												<?php echo ($magasin_supp->lib_magasin); ?>
											</option>
										<?php }
									} ?>
									</select>					
							...
							<tr>
								<td style="text-align:right; padding-right:3px;">
									...
									<?php }elseif($document->getId_etat_doc () == 61) {
										 //[61]En attente => [59]En saisie ?>
									<span id="ticket_en_saisie" style="cursor:pointer">En saisie</span>
									<div style="height:3px; line-height:3px;"></div>
									<?php }?>
								</td>
							</tr>
			...
			<script type="text/javascript">
				...
				Event.observe("code_affaire", "blur", function(evt){
					...
					if($("code_affaire").value != ""){ $("lien_recherche_avancee_code_affaire").show(); }
					else{ $("lien_recherche_avancee_code_affaire").hide();}
				}, false);
			...
			<?php if (count($_SESSION['magasins'])>1) { ?>
				Event.observe("id_magasin", "change", function(evt){
			...
			if ($document->getId_etat_doc () == 59) { //[59]En saisie => [62]Encaissé ?>
				Event.observe("ticket_a_encaisser", "click", function(evt){
					Event.stop(evt);
						maj_etat_doc (62);
					$("ticket_a_encaisser").hide();
				}, false);
			<?php }elseif($document->getId_etat_doc () == 61){ //[61]En attente => [59]En saisie ?>
				Event.observe("ticket_en_saisie", "click", function(evt){
					Event.stop(evt);
						maj_etat_doc (59);
					$("ticket_a_encaisser").hide();
				}, false);
	...
	<tr>
		...
		<table cellpadding="0" cellspacing="0" border="0" style="width:100%" id="document_contact_entete" class="document_box">
			<tr style=" line-height:20px; height:20px;" class="document_head_list">
				...
				<script type="text/javascript">
					...
					Event.observe('doc_view_contact_img', 'click',  function(){
						if ($("ref_contact").value!="" && $("ref_contact").value!="NULL") {
							page.verify('affaires_affiche_fiche','annuaire_view_fiche.php?ref_contact=<?php echo $document->getRef_contact();?>','true','sub_content');
						}
					}, false);
					
					Event.observe('doc_ref_contact_img', 'click',  function(){
						show_mini_moteur_contacts ('docu_maj_contact', '\'<?php echo $document->getRef_doc();?>\'');
						preselect ('<?php echo $CLIENT_ID_PROFIL;?>', 'id_profil_m');
					}, false);
					
					Event.observe('doc_sup_contact_img', 'click',  function(){
						docu_maj_contact("<?php echo $document->getRef_doc();?>", "");
					}, false);
			...
			<tr>
				...
				<td>
					<div style="width:17px; height:19px"></div>
				</td>
			</tr>
			<tr>
				<td colspan="3">
							...
							<td style="width:250px;">
								<div  class="doc_contact_class_input" id="view_doc_adresse_resume" style="display:">
									...
									<div id="aff_doc_adresse_edition" style="display:none">
										<textarea name="adresse_contact" id="adresse_contact" title="Adresse" class="classinput_xsize" rows="<?php if (stristr($_SERVER["HTTP_USER_AGENT"], "firefox")) { echo "1"; } else { echo "2"; } ?>">
											<?php echo  ($document->getText_adresse_contact());?>
										</textarea>
										<div id="adresse_contact_old" style="display:none">
											<?php echo ($document->getText_adresse_contact());?>
										</div>
										<div style="height:3px;"></div>
										...
										<div id="code_postal_contact_old" style="display:none">
											<?php echo ($document->getCode_postal_contact());?>
										</div>
										<div style="height:3px;"></div>
										...
										<div style="position:relative; top:0px; left:0px; width:100%; height:0px;">
											...
											<div id="choix_ville_contact"  class="choix_complete_ville"></div>
										</div>
										...
										<script type="text/javascript">
											...
											Event.observe('ville_contact', 'focus',  function(evt){
												start_commune("code_postal_contact", "ville_contact", "choix_ville_contact", "iframe_choix_ville_contact", "liste_ville_doc_contact.php?cp=");
											}, false);
												
											Event.observe('id_pays_contact', 'focus',  function(){
												delay_close("choix_ville_contact", "iframe_choix_ville_contact");
											}, false);
						...
						<tr>
							<td>
								<div style="position:relative; top:-21px; left:0px; width:100%; height:0px;">
									...
									<div id="choix_liste_choix_adresse_contact"  class="choix_liste_choix_coordonnee" style="display:none"></div>
								</div>
							</td>
							<td>
							</td>
		...
		<script type="text/javascript">
			...
			if ($document->getApp_tarifs()) { ?>
				...
				else { $("prix_afficher_ttc").checked = "checked"; }
			<?php } ?>
		</script>
		</div>
		</div>
...
<script type="text/javascript">
<?php if ($document->getId_etat_doc () == 62 ) { ?>
	$("doc_menu_7").show();
	<?php } else { ?>
	$("doc_menu_7").hide();
<?php } ?>
