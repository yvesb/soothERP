...
<table width="100%"  border="0" cellspacing="0" cellpadding="0" id="tableresult">
			...
			<tr >
				...
				<td style="width:19%"> <hr style="height: 2px;margin:0;padding:0;color:#000000;background-color:#000000;border:0"/>   </td>
				<td style="width:17%"> <hr style="height: 2px;margin:0;padding:0;color:#000000;background-color:#000000;border:0"/>   </td>
				<td style="width:6%"> <hr style="height: 2px;margin:0;padding:0;color:#000000;background-color:#000000;border:0"/>  </td>
	...
	?>
		...
		<tr class="colorise1">
				...
				<td style="width:15%; text-align:center;">
					...
					<a href="#" id="aff_resume_stock_<?php echo ($article->ref_doc_line);?>"><?php echo ($article->qte)."x";?></a>
					</span>
					<script type="text/javascript">
				...
				<div id="aff_prix_ht_<?php echo $colorise;?>" style="display:<?php if ($_REQUEST["app_tarifs"] != "HT") { echo "none";} ?>">
					<?php { echo price_format((($article->pu_ht)*($article->qte)), $TARIFS_NB_DECIMALES, ",", ""	)." ".$MONNAIE[1]." HT"; }?>
				</div>
				<div id="aff_prix_ttc_<?php echo $colorise;?>" style="display:<?php if ($_REQUEST["app_tarifs"] != "TTC") { echo "none";} ?>">
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0" id="tableresult">
			...
			<tr class="colorise1">
				<td style="width:2% ; vertical-align:middle; text-align:left">
				</td>
				...
				<td style="width:18%;text-align:right; font-weight:bold">
					<div id="aff_montant_ht_<?php echo $montant;?>" style="display:<?php if ($_REQUEST["app_tarifs"] != "HT") { echo "none";}?>">
						<?php	 { $montant_total_page_ht +=$fiche->montant_ht; echo price_format($fiche->montant_ht, $TARIFS_NB_DECIMALES, ",", ""	)." ".$MONNAIE[1]." HT";  }?>
					</div>
					<div id="aff_montant_ttc_<?php echo $montant;?>" style="display:<?php if ($_REQUEST["app_tarifs"] != "TTC") { echo "none";}?>">
...
if ($DOCUMENT_RECHERCHE_MONTANT_TOTAL) {
	...
	<tr class="colorise0">
		...
		Afficher les tarifs 
			<input  name="taxation" id="taxation_ht" type="radio"  value="HT" <?php if ($_REQUEST["app_tarifs"] == "HT") {?> checked="checked"<?php }?>>HT
		...
		<div id="aff_total_ht" style="display:<?php if ($_REQUEST["app_tarifs"] != "HT") { echo "none";}?>">
		<?php echo price_format($montant_total_page_ht, $TARIFS_NB_DECIMALES, ",", ""	)." ".$MONNAIE[1]." HT"?>
		</div>
		<div id="aff_total_ttc" style="display:<?php if ($_REQUEST["app_tarifs"] != "TTC") { echo "none";}?>">
