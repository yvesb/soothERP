...
?>
				...
				while ($art_categ = current($list_art_categ) ){
					...
					<tr id="tr_<?php echo ($art_categ->ref_art_categ)?>_cata_m" class="list_art_categs"><td width="5px">
							...
							if ($i==$art_categ->indentation) {
								if (key($list_art_categ)!="") {
									if ($art_categ->indentation < current($list_art_categ)->indentation) {
										...
										<img src="<?php echo $DIR.$_SESSION['theme']->getDir_theme()?>images/<?php if ($DEFAUT_LEVEL_CATEG_AFFICHED > $i) {echo "collapse";} else {echo "extend";}?>.gif" width="14px" id="extend_<?php echo $art_categ->ref_art_categ?>_cata_m"/>
										<img src="<?php echo $DIR.$_SESSION['theme']->getDir_theme()?>images/<?php if ($DEFAUT_LEVEL_CATEG_AFFICHED > $i) {echo "extend";} else {echo "collapse";}?>.gif" width="14px" id="collapse_<?php echo $art_categ->ref_art_categ?>_cata_m" style="display:none"/></a>
						...
						<a href="#" id="mod_<?php echo ($art_categ->ref_art_categ)?>_cata_m" style="display:block; width:100%"><?php echo htmlentities($art_categ->lib_art_categ)?>						</a>
						</td>
					</tr>
				</table>
				<?php 
...
foreach ($list_art_categ  as $art_categ){
	?>
	pre_start_art_categ ("<?php echo ($art_categ->ref_art_categ)?>", "<?php echo htmlentities($art_categ->lib_art_categ)?>", "_cata_m");
	<?php 
}
?>
</script>