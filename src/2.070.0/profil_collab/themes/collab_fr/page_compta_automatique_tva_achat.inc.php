...
<p class="titre">Numéros de compte associés aux taux de TVA décaissés </p>
...
<div style="padding-left:10px; padding-right:10px">
				...
				foreach ($tvas  as $tva){
					...
					<td style="text-align:center">
					<span id="aff_num_compte_achat_<?php echo $tva['id_tva'];?>" style="cursor:pointer; text-decoration:underline; padding-right:15px;" ><?php if ($tva['num_compte_achat']) { echo ($tva['num_compte_achat']);} else { echo $DEFAUT_COMPTE_TVA_ACHAT;}?></span>
					<script type="text/javascript">
					Event.observe("aff_num_compte_achat_<?php echo $tva['id_tva'];?>", "click",  function(evt){Event.stop(evt); ouvre_compta_plan_mini_moteur();
					charger_compta_plan_mini_moteur ("compte_plan_comptable_search.php?cible=tva&cible_id=<?php echo $tva['id_tva'];?>&retour_value_id=aff_num_compte_achat_<?php echo $tva['id_tva'];?>&retour_lib_id=lib_aff_num_compte_achat_<?php echo $tva['id_tva'];?>&type=achat&indent=num_compte_achat&num_compte=<?php echo $DEFAUT_COMPTE_TVA_ACHAT;?>");}, false);
					</script>
					</td>
				<td>&nbsp;</td>
					<td>
					<span id="lib_aff_num_compte_achat_<?php echo $tva['id_tva'];?>">
					<?php if ($tva['lib_compte_achat']) { echo $tva["lib_compte_achat"];} else {  $lcpt = new compta_plan_general($DEFAUT_COMPTE_TVA_ACHAT); echo $lcpt->getLib_compte();}?>
					</span>
					</td>
					</tr>
					<tr>
