...
class pdf_journaux_tresorerie extends PDF_etendu {
	var $code_pdf_modele = "journaux_tresorerie";
...
protected function create_pdf_entete () {
	...
	$this->Cell (95, 5, $this->journal, 0, 0, 'L');
...
protected function create_pdf_corps_line ($line) {
	...
	if (!isset($line->type_of_line)) 	{
		...
		$this->Cell($this->LARGEUR_COL_DAT, $hauteur, date_Us_to_Fr($line->date_operation), $cadre, 0, 'C');
		$this->Cell($this->LARGEUR_COL_CBIL, $hauteur, $line->numero_compte, $cadre, 0, 'C');
		$this->Cell($this->LARGEUR_COL_CTIER, $hauteur, $line->compte_tier, $cadre, 0, 'L');
		$this->Cell($this->LARGEUR_COL_LIB, $hauteur, $line->libelle, $cadre, 0, 'C');
		if (isset ($line->montant) && $line->montant < 0) {
			$this->Cell($this->LARGEUR_COL_DEB, $hauteur, number_format($line->montant, $TARIFS_NB_DECIMALES, ".", ""	)." ".$MONNAIE[0], $cadre, 0, 'R');
		} else {
			$this->Cell($this->LARGEUR_COL_DEB, $hauteur, "", $cadre, 0, 'R');
		}
		if (isset ($line->montant) && $line->montant > 0) {
			$this->Cell($this->LARGEUR_COL_CRE, $hauteur, number_format($line->montant, $TARIFS_NB_DECIMALES, ".", ""	)." ".$MONNAIE[0], $cadre, 0, 'R');
		} else {
