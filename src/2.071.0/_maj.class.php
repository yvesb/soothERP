...
// Upload un répertoire complet
    function ftp_download_dir() {
        ...
        if (!is_dir($this->tmp_files_dir . "files/")) {
            mkdir($this->tmp_files_dir . "files/");
        }
        $dir_list = $this->install_dirs;
        foreach ($dir_list as $dir) {
            if (!is_dir($this->tmp_files_dir . "files/" . $dir['SRC'])) {
                @mkdir($this->tmp_files_dir . "files/" . $dir['SRC']);
            }
...
// Effectue une copie exacte d'un dossier vers un autre
    function synchronise_dir($source_dir, $dest_dir) {
        ...
        for ($i = 2; $i < count($files); $i++) {
            ...
            if (in_array($new_dest_dir, $this->do_not_synchro_dirs)) {
                continue;
            }
