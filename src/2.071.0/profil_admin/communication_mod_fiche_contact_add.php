...
if (!isset($_REQUEST["id_profil"]) || !$_REQUEST["id_profil"]) {$GLOBALS['_ALERTES']['choisir_id_type_contact'] = 1;}
...
if (!count($GLOBALS['_ALERTES'])) {
  ...
  if (isset($class_name) && isset($config_name) && ($class_name == $config_name) && !count($GLOBALS['_ALERTES'])) {
    $query = "INSERT INTO pdf_modeles (id_pdf_type, lib_modele, desc_modele , code_pdf_modele)
              VALUES ('4', '".addslashes($_REQUEST["lib_modele"])."', '".addslashes($_REQUEST["desc_modele"])."',
                      '".$class_name."' ) ";
    $bdd->exec ($query);
    $id_pdf_modele= $bdd->lastInsertId();
    $query = "INSERT INTO annuaire_modeles_pdf (id_pdf_modele, id_profil, `usage`)
              VALUES ('".$id_pdf_modele."', '".$_REQUEST["id_profil"]."', 'inactif' ) ";
    $bdd->exec ($query);
...
include ($DIR.$_SESSION['theme']->getDir_theme()."page_communication_mod_fiche_contact_add.inc.php");
?>  