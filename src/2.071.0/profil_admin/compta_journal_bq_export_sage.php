...
//echo count($fiches);
	...
	foreach ($fiches as $fiche) {
		...
		echo $journal_exp.date("dmy",strtotime($fiche->date_move)).str_pad($fiche->numero_compte, 6, "0", STR_PAD_RIGHT).substr($fiche->ref_doc,0,3).substr($fiche->ref_doc,11,16).str_pad(str_replace(";","",substr($fiche->lib_move,0,35)),35, " ", STR_PAD_RIGHT).$DorC.str_pad(abs(number_format($fiche->montant_move, $TARIFS_NB_DECIMALES, ".", ""	)),14, "0", STR_PAD_LEFT)."\n";
