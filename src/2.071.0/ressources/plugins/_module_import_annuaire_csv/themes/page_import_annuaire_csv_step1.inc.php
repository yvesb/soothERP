...
<br />&nbsp;
<form action="<?php echo $PROFILE_DIR.$PLUGINS_REP; ?>_module_import_annuaire_csv/import_annuaire_csv_step1_done.php" enctype="multipart/form-data" method="POST" id="import_annuaire_csv_done" name="import_annuaire_csv_done" target="formFrame" class="classinput_nsize" />
...
<table class="contactview_corps" style=" width:100%">
	...
	foreach ($entete_corresp['champs'] as $champs_val) {
		...
		<tr>
			...
			<script type="text/javascript">
			<?php if ($preselect) {?>
					page.verify('import_annuaire_csv_correspondances','<?php echo $PROFILE_DIR.$PLUGINS_REP; ?>_module_import_annuaire_csv/import_annuaire_csv_correspondances.php?lmb_col=<?php echo $champs_val["id"];?>&csv_col=\'+$("<?php echo $champs_val["id"];?>").options[$("<?php echo $champs_val["id"];?>").selectedIndex].value\'','true','correspondances_<?php echo $champs_val["id"];?>');
			<?php } ?>
