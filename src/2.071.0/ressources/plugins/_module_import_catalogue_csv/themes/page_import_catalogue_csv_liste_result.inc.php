...
function barre_navigation($nbtotal, $nbenr, $cfg_nbres_ppage, $debut, $cfg_nb_pages, $idformtochange, $fonctiontolauch) {
	...
	$lien_off	 = "&nbsp;{lien}&nbsp;";
	$gaucheonimg = image_html($PROFILE_DIR. $_SESSION['theme']->getCode_gtheme() . 'images/gauche_on.gif');
	$droiteonimg = image_html($PROFILE_DIR . $_SESSION['theme']->getCode_gtheme() . 'images/droite_on.gif');
	$gaucheoffimg = image_html($PROFILE_DIR. $_SESSION['theme']->getCode_gtheme() . 'images/gauche_off.gif');
	$droiteoffimg = image_html($PROFILE_DIR . $_SESSION['theme']->getCode_gtheme() . 'images/droite_off.gif');
	// --------------------------------------------------------------------
	// début << .
	// --------------------------------------------------------------------
...
$barre_nav .= barre_navigation($nb_fiches, $form['page_to_show'], $form['fiches_par_page'], $debut, $cfg_nb_pages, 'page_to_show_s', 'catalogue_import()');
...
if (count($array_retour)) {
	for ($i = 0; $i < 1; $i++) {
		...
		foreach ($array_retour[$i] as $k => $fiche) {
			...
			foreach ($import_catalogue_csv['liste_entete'] as $entete) {
			...
			foreach ($tarifs_liste as $tarif) {
				if ("id_tarif_" . $tarif->id_tarif == $k) {
					echo $tarif->lib_tarif;
					break;
				}
			}
			foreach ($tarifs_liste as $tarif) {
				if ("id_tarif_qte_" . $tarif->id_tarif == $k) {
					echo "Qté " . $tarif->lib_tarif;
					break;
				}
			}
			if ("ref_art_categ" == $k) {
				echo "Catégorie d'article ";
			}
			?></div></td>
			<?php
		}
		?>
				...
				<tr id="ligne_<?php echo $i; ?>" class="<?php echo $class_colorise ?>">
					...
					<td style="text-align:left" >
							<?php if (isset($array_retour[$i]["averti"])) { ?><img src="<?php echo $PROFILE_DIR.$_SESSION['theme']->getCode_gtheme(); ?>images/ding.gif" title="Cette ligne pourrait ne pas être totalement importée"/><?php } ?>
							<?php if (isset($array_retour[$i]["alerte"])) { ?><img src="<?php echo $PROFILE_DIR.$_SESSION['theme']->getCode_gtheme(); ?>images/alerte.gif" title="Cette ligne ne sera pas importée"/><?php } ?>
					</td>
					<td style="text-align:left" >
						<img src="<?php echo $PROFILE_DIR.$_SESSION['theme']->getDir_gtheme() ?>images/supprime.gif" style="cursor:pointer" id="del_ligne_<?php echo $i; ?>"/>

						<script type="text/javascript">

							Event.observe("del_ligne_<?php echo $i; ?>", "click", function (evt) {
								Event.stop(evt);
								supprime_import_ligne_catalogue("<?php echo $i; ?>");
							});
						</script>
					</td>
				<?php
...
?>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="">
			<tr>
				<td rowspan="2" style="width:33px">
					<img src="<?php echo $PROFILE_DIR . $_SESSION['theme']->getDir_gtheme() ?>images/arrow_ltr.png" />
				</td>
				<td style="height:4px; line-height:4px">
					<img src="<?php echo $PROFILE_DIR . $_SESSION['theme']->getDir_gtheme() ?>images/blank.gif" height="4px" width="100%"/>
				</td>
				<td style="height:4px; line-height:4px">
					<img src="<?php echo $PROFILE_DIR . $_SESSION['theme']->getDir_gtheme() ?>images/blank.gif" height="4px" width="100%"/>
				</td>
		...
		<script type="text/javascript">
			...
			Event.observe("all_coche_s", "click", function (evt) {
				Event.stop(evt);
				coche_line_catalogue_import("coche", "_s", <?php echo $i; ?>);
			});
			Event.observe("all_decoche_s", "click", function (evt) {
				Event.stop(evt);
				coche_line_catalogue_import("decoche", "_s", <?php echo $i; ?>);
			});
			Event.observe("all_inv_coche_s", "click", function (evt) {
				Event.stop(evt);
				coche_line_catalogue_import("inv_coche", "_s", <?php echo $i; ?>);
			});

			Event.observe("coche_action_s", "change", function (evt) {
				if ($("coche_action_s").value !== "") {
					action_catalogue_import($("coche_action_s").value, "_s", <?php echo $i; ?>);
				}
			});
		</script>
	</div>
