...
function catalogue_import() {
		var AppelAjax = new Ajax.Updater(
									"resultat_imports", 
									PLUGINS_REP + "_module_import_catalogue_csv/import_catalogue_csv_liste_result.php", {
									method: 'post',
									asynchronous: true,
...
function action_catalogue_import(action_selection, second_id , length_list) {
		...
		if (action_selection == "import") {
				var AppelAjax = new Ajax.Request(
													PLUGINS_REP + "_module_import_catalogue_csv/import_catalogue_csv_action.php?fonction_generer="+action_selection+liste_doc, 
													{
													evalScripts: true, 
													onLoading: S_loading,
													parameters: { recherche: '1', ref_art_categ: $F('ref_art_categ_s'), page_to_show: $F('page_to_show_s'), orderby: $F('orderby_s'), orderorder: $F('orderorder_s')},
...
function catalogue_import_maj_limite(limite) {
		var AppelAjax = new Ajax.Request(
									PLUGINS_REP + "_module_import_catalogue_csv/import_catalogue_csv_maj_limite.php", {
									method: 'post',
									asynchronous: true,
									contentType:  'application/x-www-form-urlencoded',
									encoding:     'UTF-8',
...
function supprime_import_ligne_catalogue (ligne) {
	...
	Event.observe("bouton1", "click", function () {
		var AppelAjax = new Ajax.Request(
											PLUGINS_REP + "_module_import_catalogue_csv/import_catalogue_csv_action.php?fonction_generer=supprimer&id_rec"+ligne+"="+ligne, 
											{
											evalScripts: true,
											onLoading: S_loading,
											onSuccess: function (requester){
