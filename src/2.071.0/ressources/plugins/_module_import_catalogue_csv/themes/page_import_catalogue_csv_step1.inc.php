...
<div>
Sélectionnez les correspondances SoothERP avec les colonnes du fichier CSV importé.
<br />&nbsp;
<form action="<?php echo $PROFILE_DIR.$PLUGINS_REP; ?>_module_import_catalogue_csv/import_catalogue_csv_step1_done.php" enctype="multipart/form-data" method="POST" id="import_catalogue_csv_done" name="import_catalogue_csv_done" target="formFrame" class="classinput_nsize" />
...
<span style="text-align: left; float:left; font-style:italic">Indiquez la catégorie d'article qui sera utilisée si aucune catégorie n'est définie:</span>
		...
		<tr>
			...
			<img src="<?php echo $PROFILE_DIR.$_SESSION['theme']->getDir_gtheme()?>images/ajouter.gif" id="v_correspondances_ref_art_categ" style="cursor:pointer; display:<?php if (!$preselect) {?>none<?php } ?>" />
			<img src="<?php echo $PROFILE_DIR.$_SESSION['theme']->getDir_gtheme()?>images/moins.gif" id="unv_correspondances_ref_art_categ" style="display:none; cursor:pointer" />
			<script type="text/javascript">
			<?php if ($preselect) {?>
					page.verify('import_catalogue_csv_correspondances','<?php echo $PROFILE_DIR.$PLUGINS_REP; ?>_module_import_catalogue_csv/import_catalogue_csv_correspondances.php?lmb_col=ref_art_categ&csv_col=\'+$("ref_art_categ").options[$("ref_art_categ").selectedIndex].value\'','true','correspondances_ref_art_categ');
			<?php } ?>
				...
				Event.observe("v_correspondances_ref_art_categ", "click",  function(evt){
					...
					if ($("correspondances_ref_art_categ").innerHTML === "") {
						page.verify('import_catalogue_csv_correspondances','<?php $PROFILE_DIR.$PLUGINS_REP; ?>_module_import_catalogue_csv/import_catalogue_csv_correspondances.php?lmb_col=ref_art_categ&csv_col=\'+$("ref_art_categ").options[$("ref_art_categ").selectedIndex].value\'','true','correspondances_ref_art_categ');
					}
...
<table class="contactview_corps" style=" width:100%">
	...
	foreach ($entete_corresp["champs"] as $champs_val) {
		...
		<tr>
			...
			<?php if (isset($champs_val['id_type'])) {?>
			<img src="<?php echo $PROFILE_DIR.$_SESSION['theme']->getDir_gtheme()?>images/ajouter.gif" id="v_correspondances_<?php echo $champs_val['id'];?>" style="cursor:pointer; display:<?php if (!$preselect) {?>none<?php } ?>" />
			<img src="<?php echo $PROFILE_DIR.$_SESSION['theme']->getDir_gtheme()?>images/moins.gif" id="unv_correspondances_<?php echo $champs_val['id'];?>" style="display:none; cursor:pointer" />
			<script type="text/javascript">
			<?php if ($preselect) {?>
					page.verify('import_catalogue_csv_correspondances','<?php echo $PROFILE_DIR.$PLUGINS_REP; ?>_module_import_catalogue_csv/import_catalogue_csv_correspondances.php?lmb_col=<?php echo $champs_val["id"];?>&csv_col=\'+$("<?php echo $champs_val["id"];?>").options[$("<?php echo $champs_val["id"];?>").selectedIndex].value\'','true','correspondances_<?php echo $champs_val["id"];?>');
			<?php } ?>
...
foreach ($tarifs_liste as $tarif) {
	...
	<tr>
		...
		<td>
			...
			<img src="<?php echo $PROFILE_DIR.$_SESSION['theme']->getDir_gtheme()?>images/ajouter.gif" id="v_correspondances_id_tarif_<?php echo $tarif->id_tarif;?>" style="cursor:pointer; display:<?php if (!$preselect) {?>none<?php } ?>" />
			<img src="<?php echo $PROFILE_DIR.$_SESSION['theme']->getDir_gtheme()?>images/moins.gif" id="unv_correspondances_id_tarif_<?php echo $tarif->id_tarif;?>" style="display:none; cursor:pointer" />
			<script type="text/javascript">
				...
				Event.observe("id_tarif_<?php echo $tarif->id_tarif;?>", "change",  function(evt){
					...
					page.verify('import_catalogue_csv_correspondances','<?php echo $PROFILE_DIR.$PLUGINS_REP; ?>_module_import_catalogue_csv/import_catalogue_csv_correspondances.php?lmb_col=id_tarif&id_tarif=<?php echo $tarif->id_tarif;?>&csv_col=\'+$("id_tarif_<?php echo $tarif->id_tarif;?>").options[$("id_tarif_<?php echo $tarif->id_tarif;?>").selectedIndex].value\'','true','correspondances_id_tarif_<?php echo $tarif->id_tarif;?>');
					} else {
					$("v_correspondances_id_tarif_<?php echo $tarif->id_tarif;?>").hide();
				...
				Event.observe("v_correspondances_id_tarif_<?php echo $tarif->id_tarif;?>", "click",  function(evt){
					...
					if ($("correspondances_id_tarif_<?php echo $tarif->id_tarif;?>").innerHTML == "") {
						page.verify('import_catalogue_csv_correspondances','<?php $PROFILE_DIR.$PLUGINS_REP; ?>_module_import_catalogue_csv/import_catalogue_csv_correspondances.php?lmb_col=id_tarif&id_tarif=<?php echo $tarif->id_tarif;?>&csv_col=\'+$("id_tarif_<?php echo $tarif->id_tarif;?>").options[$("id_tarif_<?php echo $tarif->id_tarif;?>").selectedIndex].value\'','true','correspondances_id_tarif_<?php echo $tarif->id_tarif;?>');
					}
