	...
	while ($row = $req->fetchObject()) { $citys[] = $row; }
		...
		foreach ($citys as $city) {
			echo 'Event.observe("'.$_REQUEST["choix_ville"].'_'.$i.'", "mouseout",  function(){changeclassname ("'.$_REQUEST["choix_ville"].'_'.$i.'", "complete_ville");}, false);
						...
						Event.observe("'.$_REQUEST["choix_ville"].'_'.$i.'", "click",  function(){$("'.$_REQUEST["ville"].'").value="'.htmlentities($city->ville, ENT_QUOTES, "UTF-8").'"; $("'.$_REQUEST["choix_ville"].'").style.display="none"; $("'.$_REQUEST["iframe_choix_ville"].'").style.display="none";}, false);';
				$i++;
