	...
	<tr class="">
									...
									<table cellpadding="0" cellspacing="0" border="0" style="width:100%;">
										<tr>
											...
											if ($document->getId_etat_doc () == 25) {
												?>
											<img src="<?php echo $DIR.$_SESSION['theme']->getDir_theme()?>images/bt_cdf_lancer.gif" id="commande_pret" style="cursor:pointer"/>
		...
		// observateurde changement de texte dans l'entete du doc pour mise à jour des infos
			...
			if ($document->getId_etat_doc () == 25) {
				?>
				//
				Event.observe("commande_pret", "click", function(evt){Event.stop(evt); maj_etat_doc (27);
			...
			if ($document->getId_etat_doc () == 27) {
				?>
				//
				Event.observe("creer_reception", "click", function(evt){Event.stop(evt); generer_document("generer_br_fournisseur");
						$("creer_reception").hide();}, false);
			<?php
			}
			?>
