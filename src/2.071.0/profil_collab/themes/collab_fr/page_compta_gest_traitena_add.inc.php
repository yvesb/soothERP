 <table width="100%" border="0" cellspacing="4" cellpadding="2">
        ...
        <tr>
            ...
            <td style="width:45%">
                <?php echo Helper::createInputAnnu("ref_client", $cur_name,array("callback"=>"update_autorisations_compte","filtres"=>"lib=client"));
                if (!empty($cur_name)){?>  <script type="text/javascript">
                    $("ref_client").value = "<?php echo $_REQUEST['ref_contact']; ?>";
                    $("ref_client_nom_c").value ="<?php echo $cur_name; ?>";
                    update_autorisations_compte("ref_client", "ref_client_nom_c", $('ref_client').value, $('ref_client_nom_c').value);
                </script>
                <?php } ?>
            </td>
            <td>&nbsp</td>
        ...
        <tr>
            ...
            <td><span class="clic" id="add_pj_traitena">+</span></td>
            <td>&nbsp</td>
        </tr>
        <tr>
            <td>
                <span class="labelled">Destination de la traite:</span>
            </td>
            <td>
        ...
        <tr>
            <td>
                <span class="labelled">Règlement favori pour ce client :</span>
            </td>
            <td>
                <input type="checkbox" id="id_rgmnt_fav" value="" class="classinput_xsize"/>
            </td>
        </tr>
        <tr>
            <td>
    ...
    <script type="text/javascript">
        ...
        Event.observe('add_pj_traitena', "click", function(evt){
            if($("ref_client").value != ""){
        ...
        Event.observe('submit_a', "click", function(evt){
            if($("ref_client").value != ""){
                if ($("id_rgmnt_fav").checked == true){
                    $("id_rgmnt_fav").value = 1;
                }
                else{
                    $("id_rgmnt_fav").value = 0;
                }
                if ($("id_compte_src").value != "")
                    {
                        compta_traitena_add();
                        $("pop_up_traitena").hide();
                        window.location.reload();
                    }
                else
                    {
                        window.alerte.alerte_erreur('Erreur', "Le compte source n'est pas spécifié",'<input type="submit" id="bouton0" name="bouton0" value="Ok" />');
                       $("pop_up_traitena").hide();
                    }
