...
// Retourne le répertoire global du theme
...
// Retourne le répertoire CSS du theme
    final public function getDir_css($condival = 'yes') {
        global $CHOIX_THEME;
        if ($condival == "no") {
            $dir_css = "themes/" . $CHOIX_THEME . "/css/" . $_SESSION['interfaces'][$this->id_interface]->getDossier() . ""; // Répertoire css du theme et de l'interface
        } else {
            $dir_css = "themes/" . $CHOIX_THEME . "/css/"; // Répertoire du thème global css
        }
        return $dir_css;
    }

// Retourne le répertoire JS du theme
    final public function getDir_js($condival = 'yes') {
        global $CHOIX_THEME;
        if ($condival == "no") {
            $dir_js = "themes/" . $CHOIX_THEME . "/js/" . $_SESSION['interfaces'][$this->id_interface]->getDossier() . ""; // Répertoire de ce thème global
        } else {
            $dir_js = "themes/" . $CHOIX_THEME . "/js/"; // Répertoire de thème global
        }
        return $dir_js;
    }

// Retourne le répertoire IMG du theme
    final public function getDir_img($condival = 'yes') {
        global $CHOIX_THEME;
        if ($condival == "no") {
            $dir_img = "themes/" . $CHOIX_THEME . "/img/"; // Répertoire de thème global
        } else {
            $dir_img = "themes/" . $CHOIX_THEME . "/img/" . $_SESSION['interfaces'][$this->id_interface]->getDossier() . ""; // Répertoire de ce thème global
        }
        return $dir_img;
    }
