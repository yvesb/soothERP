...
abstract class Agenda {
	...
	public function __construct($ref_agenda) {
		global $bdd;
		if (!is_numeric($this->getId_type_agenda())) return false;
	...
	public function &getEvents() {
		if ($this->events == null) {
			...
			$resultat	 = $bdd->query($query);
			while ($r			 = $resultat->fetchObject()) $this->addEvent(new Event($r->ref_agenda_event));
		}
		return $this->events;
	}
	...
	public function setId_agenda_style_rule($id_agenda_style_rule) {
		global $bdd;
		if (!is_numeric($id_agenda_style_rule)) return false;
		//else
...
class AgendaReservationRessource extends Agenda {
	...
	public final static function &newAgendaReservationRessource($lib_agenda, $tabRessources, $id_agenda_style_rule) {
		...
		if ($lib_agenda == null || $lib_agenda == "" || count($tabRessources) <= 0 || !is_numeric($id_agenda_style_rule))
				return false;
...
class AgendaContact extends Agenda {
	...
	public function __construct($ref_agenda) {
		...
		if (!parent::__construct($ref_agenda)) return false;
	}

	public final static function &newAgendaContact($lib_agenda, $ref_contact, $id_agenda_style_rule) {
		global $bdd;

		if ($lib_agenda == null || $lib_agenda == "" || $ref_contact == null || $ref_contact == "" || !is_numeric($id_agenda_style_rule))
				return false;
...
class AgendaLoacationMateriel extends Agenda {
	...
	public final static function &newAgendaLoacationMateriel($lib_agenda, $ref_article, $id_agenda_style_rule) {
		...
		if ($lib_agenda == null || $lib_agenda == "" || $ref_article == null || $ref_article == "" || !is_numeric($id_agenda_style_rule))
				return false;
	...
	public function getArticle() {
		if ($this->article == null) $this->article = new article($this->ref_article);
		return $this->article;
	}
