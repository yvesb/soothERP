...
class Event {
	...
	public final static function &newEvent(&$agenda, &$event_Parent, $lib_event, $note_event, $Udate_event, $duree_event, $duree_all_day = false) {
		global $bdd;
		$newEvent = null;
		if ($agenda == null || $lib_event == null || $lib_event == "" || !is_numeric($Udate_event) || !is_numeric($duree_event) || !is_bool($duree_all_day))
		...
		if ($this->ref_event_parent == "") {
			...
			if ($this->event_parent != null) {
				...
				for ($i = 0; i < count($cousins); $i++) {
					if ($cousins[$i]->getRef_event() == $this->getRef_event()) $cousins[$i] = & $this;
					break;
				}
			}
		}
