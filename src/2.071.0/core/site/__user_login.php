...
if (isset($_REQUEST["uncache"])) {
    $uncache = "uncache=" . $_REQUEST['uncache'];
} else {
    $uncache = "";
}
...
if (isset($_REQUEST['login']) && isset($_REQUEST['code'])) {
    ...
    $id_profil = NULL;
    if (isset($_REQUEST['id_profil_force'])) {
        $id_profil = $_REQUEST['id_profil_force'];
    }

    $login_result = $_SESSION['user']->login($_REQUEST['login'], $_REQUEST['code'], $page_from, $id_profil);

    $hash = "";
    if ($page_from || $uncache) {
        $hash .= "?";
    }
    if ($uncache) {
        $hash .= "&" . $uncache;
...
//  ******************************************************
        if ($SESSION_START_BACKUP) include($CORE_DIR . "taches_auto/session_backup.php");
...
if ($MODE_IDENTIFICATION == "SELECT") {
    $query  = "SELECT ref_user, pseudo FROM users WHERE actif = 1 ORDER BY pseudo";
