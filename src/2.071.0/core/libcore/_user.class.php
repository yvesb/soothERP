...
// Constructeur
    function __construct() {
        ...
        foreach ($DEFAUT_PROFILS as $id_profil) {
            $this->allowed_profils[$id_profil] = $id_profil;
        }
...
// permissions associés à l'utilisateurs
    final private function define_permissions() {
        ...
        foreach ($this->permissions as $permission) {
            if ($permission->id_permission_parent) {
                continue;
            }           // Si il y a une permission parent, nous ne sommes pas à la racine
            if (!isset($_SESSION['profils'][$permission->id_profil])) {
                continue;
            } // Si le profil n'est pas en session, ne pas l'autoriser
