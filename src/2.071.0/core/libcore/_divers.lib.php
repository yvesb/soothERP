...
function lmb_strftime($format, $info_locale, $timestamp = -1) {
	if ($timestamp < 0) {
		$timestamp = time();
	}
...
function charge_modele_export_result_commerciaux() {
	...
	$query			 = "SELECT id_export_modele, id_export_type, lib_modele, desc_modele , code_export_modele , extension
							...
							";
	$resultat		 = $bdd->query($query);
	while ($modele_export	 = $resultat->fetchObject()) {
		$modeles_liste[] = $modele_export;
	}
	return $modeles_liste;
}
...
function dumpDB($path, $host, $db, $user, $pw) {
	...
	if (is_resource($process)) {
		...
		while (!feof($pipes[1])) $buffer .= fgets($pipes[1], 1024);
		fclose($pipes[1]);
...
function listFiles($path) {
	...
	// }  
	//}
	foreach (glob($path . "/*") as $val) array_push($files, $val);
	return $files;
}
