...
final class newsletter {
    ...
    public function __construct($id_newsletter = "") {
        ...
        if (!$newsletter = $resultat->fetchObject()) {
            return false;
        }
        ...
        if (array_search($profil->getId_profil(), $newsletter_profils) !== false) {
            ...
            switch ($profil->getId_profil()) {
                case $ADMIN_ID_PROFIL:
                    ...
                    $resultat = $bdd->query($query);
                    while ($tmp      = $resultat->fetchObject()) {
                        $liste_email[$tmp->email] = $tmp;
                    }
                    break;
                case $COLLAB_ID_PROFIL:
                    $collab_fct = explode(";", $criteres[0]);
                    if (count($collab_fct) && $collab_fct[0] != "") {
                        $query_where = " && uf.id_fonction IN ( " . implode(",", $collab_fct) . " )";
                    }
                    $query    = "SELECT TRIM(c.email)email, a.nom
										FROM coordonnees c
											LEFT JOIN annuaire a ON a.ref_contact = c.ref_contact
                ...
                case $CLIENT_ID_PROFIL:
                    ...
                    if (count($client_categorie) && $client_categorie[0] != "") {
                        $query_where .= " && ac.id_categorie IN ( " . implode(",", $client_categorie) . " )";
                    }

                    $client_type = explode(";", $criteres[1]);
                    if (count($client_type) && $client_type[0] != "") {
                        $query_where .= " && anc.type_client IN ( '" . implode("','", $client_type) . "' )";
                    }

                    $client_categ = explode(";", $criteres[2]);
                    if (count($client_categ) && $client_categ[0] != "") {
                        $query_where .= " && anc.id_client_categ IN ( " . implode(",", $client_categ) . " )";
                    }
                ...
                case $FOURNISSEUR_ID_PROFIL:
                    ...
                    if (count($fourn_cat) && $fourn_cat[0] != "") {
                        $query_where = " && af.id_fournisseur_categ IN ( " . implode(",", $fourn_cat) . " )";
                    }
                    $query    = "SELECT TRIM(c.email)email, a.nom
										FROM coordonnees c
											LEFT JOIN annuaire a ON a.ref_contact = c.ref_contact
...
function verifier_code_unique($code_unique, $email, $id_newsletter) {
    ...
    if (!file_exists($CONFIG_DIR . "newsletter.config.php")) {
        return false;
    } // L'ERREUR N'EST PAS GEREE DANS LE CODE : Trouver un moyen propre de le faire proprement
