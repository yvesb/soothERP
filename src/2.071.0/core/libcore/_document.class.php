...
// Changement du contact d'un document
    public function maj_contact($ref_contact) {
        ...
        while ($rgmt            = $resultat->fetchObject()) {
            ...
            if ($rgmt->nb_docs > 1) {
                $rgmts_to_deli[] = $rgmt->ref_reglement;
            } else {
                $rgmts_to_update[] = $rgmt->ref_reglement;
            }
        }
        if ($rgmts_to_update) {
            $list_to_update = "''";
            foreach ($rgmts_to_update as $ref_rgmt) {
                $list_to_update .= ",'" . $ref_rgmt . "'";
            }
        ...
        if ($rgmts_to_deli) {
            $list_to_deli = "''";
            foreach ($rgmts_to_deli as $ref_rgmt) {
                $list_to_deli .= ",'" . $ref_rgmt . "'";
            }
...
// Fusion de documents
    public function fusion_doc($second_ref_doc) {
        ...
        global $bdd;
        if (!isset($second_ref_doc)) {
            return false;
        }

        $second_document = open_doc($second_ref_doc);

        if (!$this->check_allow_fusion($second_document)) {
            return false;
        }
...
// Changement de l'état du document
    final public function maj_etat_doc($new_etat_doc) {
        ...
        if ($this->id_etat_doc == $new_etat_doc) {
            return false;
        }
        if (!$new_etat_doc = $this->check_maj_etat($new_etat_doc)) {
            return false;
        }
...
// Chargement du contenu du document
    public function charger_contenu() {
        ...
        for ($i = 0; $i < count($tmp_contenu); $i++) {
            if ($tmp_contenu[$i]->ref_doc_line_parent) {
                continue;
            }
            $this->contenu[] = $tmp_contenu[$i];
            for ($j = 0; $j < count($tmp_contenu); $j++) {
                if ($tmp_contenu[$j]->ref_doc_line_parent != $tmp_contenu[$i]->ref_doc_line) {
                    continue;
                }
        ...
        for ($i = 0; $i < count($this->contenu); $i++) {
            ...
            if (!isset($this->tvas[$this->contenu[$i]->tva])) {
                $this->tvas[$this->contenu[$i]->tva] = 0;
            }
        ...
        for ($i = 0; $i < count($this->contenu); $i++) {
            ...
            if ($this->contenu[$i]->gestion_sn) {
                $this->contenu[$i]->sn = array();
            }
            // Remplissage du tableau
            foreach ($numeros as $sn) {
                if ($this->contenu[$i]->ref_doc_line != $sn->ref_doc_line) {
                    continue;
                }
    ...
    protected function charger_line($ref_doc_line) {
        ...
        // *****************************************
        // Chargement des numéros de serie
        if (!$GESTION_SN || !$this->GESTION_SN || !$doc_line->gestion_sn) {
            return $doc_line;
        }
        $query_sn_infos_supp = $this->doc_line_sn_infos_supp();
...
// Chargement du contenu "matériel" du document
    public function charger_contenu_materiel() {
        ...
        for ($i = 0; $i < count($this->contenu_materiel); $i++) {
            // Création du tableau de numéros de série
            if ($this->contenu_materiel[$i]->gestion_sn) {
                $this->contenu_materiel[$i]->sn = array();
            }
            // Remplissage du tableau
            foreach ($numeros as $sn) {
                if ($this->contenu_materiel[$i]->ref_doc_line != $sn->ref_doc_line) {
                    continue;
                }
...
// Ajout d'un article depuis le catalogue
    protected function add_line_article($infos) {
        ...
        if (isset($infos['pu_ht'])) {
            $pu_ht = $infos['pu_ht'];
        } else {
            $pu_ht = $this->select_article_pu($article, $qte);
        }
        ...
        if (isset($infos['remise'])) {
            ...
            if (!$res    = $this->select_infos_article_pcotation($article, $qte))
                    $remise = 0;
            else $remise = floatval($res->remise);
        }
        if (isset($infos['ref_doc_line_parent'])) {
            $ref_doc_line_parent = $infos['ref_doc_line_parent'];
        } else {
            $ref_doc_line_parent = "";
        }
        $visible = 1;
        if ($ref_doc_line_parent || (isset($infos['visible']) && $infos['visible'])) {
            $visible = 0;
        }
        ...
        if (isset($infos['tva']) && is_numeric($infos['tva'])) {
            ...
            if (!$ASSUJETTI_TVA && ($this->ID_TYPE_DOC == $DEVIS_CLIENT_ID_TYPE_DOC || $this->ID_TYPE_DOC == $COMMANDE_CLIENT_ID_TYPE_DOC || $this->ID_TYPE_DOC == $LIVRAISON_CLIENT_ID_TYPE_DOC || $this->ID_TYPE_DOC == $FACTURE_FOURNISSEUR_ID_TYPE_DOC)) {
                $article_taux_tva = 0;
            }
...
//Savoir si c'est une taxe (suivant le lib_taxe)
    public function is_taxe($lib_taxe) {
        ...
        if (!empty($tmp->code_taxe)) return true;
        else return false;
    }
...
//  ******************************************************
    ...
    public function maj_line_qte($ref_doc_line, $new_qte) {
        ...
        if (!$this->check_allow_maj_line_qte()) {
            return false;
        }
...
// Supprime plusieurs lignes de contenu
    public function delete_multiples_lines($lines) {
        ...
        if (!$liste_lines = $this->get_liste_of_lines($lines)) {
            return false;
        }
...
// Reset le prix unitaire d'une ligne
    public function reset_pu_ht($lines) {
        ...
        while ($tmp      = $resultat->fetchObject()) {
            if (define_type_of_line($tmp->ref_article) != "article") {
                continue;
            }
...
// Selection du prix de vente adaptï¿½ au contact
    public function select_infos_article_pcotation($article, $qte) {
        ...
        if ($USE_COTATIONS) {
            ...
            if (!empty($cotation_article)) return $cotation_article;
            else return false;
        } else {
            return false;
        }
...
//				}
    ...
    public function copie_content($doc_cible) {
        ...
        for ($i = 0; $i < count($this->contenu); $i++) {
            if (!in_array($this->contenu[$i]->ref_doc_line, $GLOBALS['_OPTIONS']['CREATE_DOC']['doc_lines']) && !in_array($this->contenu[$i]->ref_doc_line_parent, $GLOBALS['_OPTIONS']['CREATE_DOC']['doc_lines'])) {
                continue;
            }
...
// $line correspond à une ligne provennant de charger_contenu
    public function copie_line_to_doc($doc, $line) {
        ...
        if ($new_line->type_of_line == "taxe" && !$new_line->ref_doc_line_parent) {
            return false;
        }

        // Informations supplémentaires pour la ligne
        if (!$this->action_before_copie_line_to_doc($doc, $line)) {
            unset($tab_correspondance[$new_line->old_ref_doc_line]);
            return false;
        }

        //gestion du taux de tva
        if (!$ASSUJETTI_TVA && ($this->ID_TYPE_DOC == $DEVIS_CLIENT_ID_TYPE_DOC || $this->ID_TYPE_DOC == $COMMANDE_CLIENT_ID_TYPE_DOC || $this->ID_TYPE_DOC == $LIVRAISON_CLIENT_ID_TYPE_DOC || $this->ID_TYPE_DOC == $FACTURE_CLIENT_ID_TYPE_DOC)) {
            $new_line->tva = 0;
        }

        // gestion du pa_ht
        if (empty($new_line->pa_forced)) {
            $new_line->pa_ht     = $article->getPaa_ht();
            $new_line->pa_forced = 0;
        }
...
// Fonction permetant d'insérer les informations complémentaires d'une ligne lors d'une copie
    protected function action_before_copie_line_to_doc($new_doc, $line) {
        return true;
    }
    ...
    public function copie_line_from_lines($lines, $new_ref_doc_line_parent = "", $old_ref_doc = "") {
        ...
        if ($document_source = open_doc($old_ref_doc)) {
            $query_infos_supp = $document_source->getDoc_line_infos_supp();
        }
        // *************************************************
        ...
        $resultat2 = $bdd->query($query);
        while ($tmp       = $resultat2->fetchObject()) {
            $sns[$tmp->ref_doc_line][] = $tmp->numero_serie;
        }
        ...
        while ($line = $resultat1->fetchObject()) {
            ...
            while ($tmp          = $resultat3->fetchObject()) {
                $lines_enfant[] = $tmp->ref_doc_line;
            }
            if (count($lines_enfant)) {
                $this->copie_line_from_lines($lines_enfant, $new_line->ref_doc_line);
            }
        }
...
// Ajoute le contenu au stock
    public function add_content_to_stock($id_stock = 0) {
        ...
        if (!$this->contenu_materiel_loaded) {
            $this->charger_contenu_materiel();
        }
...
// Ajoute le service par abo
    public function add_service_abo() {
        if (!$this->contenu_service_abo_loaded) {
            $this->charger_contenu_service_abo();
        }
...
// Ajoute le service a la conso
    public function add_service_conso() {
        if (!$this->contenu_service_conso_loaded) {
            $this->charger_contenu_service_conso();
        }
...
// Va rechercher les factures à régler, les avoirs en cours, et les règlements non lettrés
    public function get_infos_facturation($montant_positif = 1) {
        ...
        switch ($this->ID_TYPE_DOC) {
            case 1: case 2: case 3: case 4: case 15: $type = "FAC";
                break;
            case 5: case 6: case 7: case 8: $type = "FAF";
                break;
        }
...
//  ******************************************************
    ...
    function getLiaisons_possibles() {
        if (!$this->liaisons_possibles_loaded) {
            $this->charger_liaisons_possibles();
        }
    ...
    function getCommerciaux() {
        if (!$this->commerciaux_loaded) {
            $this->charger_commerciaux();
        }
    ...
    function getid_stock_search() {
        global $_SESSION;
        if (isset($this->id_stock)) {
            return $this->id_stock;
        }
        if (isset($this->id_stock_source)) {
...
//Affichage des echeances et des reglements effectués
    public function get_conditions_reglement($line_ech_max, $line_regl_max) {
        ...
        if ($this->getCODE_DOC() != "") $type_doc           = $this->getCODE_DOC();
        else $type_doc           = "";
