...
if (!isset($_REQUEST["duree_aff_doc_cdc"]) || !is_numeric($_REQUEST["duree_aff_doc_cdc"])) {
    $GLOBALS['_ALERTES']['choisir_duree_aff_doc_cdc'] = 1;
}
if (!isset($_REQUEST["duree_aff_doc_fac"]) || !is_numeric($_REQUEST["duree_aff_doc_fac"])) {
    $GLOBALS['_ALERTES']['choisir_duree_aff_doc_fac'] = 1;
}
if (!isset($_REQUEST["mail_envoi_inscriptions"])) {
    $GLOBALS['_ALERTES']['choisir_mail_envoi_inscriptions'] = 1;
}
//if (!isset($_FILES['img_logo']) || empty($_FILES['img_logo']['tmp_name'])) {$GLOBALS['_ALERTES']['choisir_img_logo'] = 1;}
if (!isset($_REQUEST['sujet_inscription_validation']) || strlen($_REQUEST['sujet_inscription_validation']) < 1) {
    $GLOBALS['_ALERTES']['choisir_sujet_inscription_validation'] = 1;
}
if (!isset($_REQUEST['sujet_inscription_validation_final']) || strlen($_REQUEST['sujet_inscription_validation_final']) < 1) {
    $GLOBALS['_ALERTES']['choisir_sujet_inscription_validation_final'] = 1;
}
if (!isset($_REQUEST['sujet_modification_validation']) || strlen($_REQUEST['sujet_modification_validation']) < 1) {
    $GLOBALS['_ALERTES']['choisir_sujet_modification_validation'] = 1;
}
if (!isset($_REQUEST['sujet_modification_validation_final']) || strlen($_REQUEST['sujet_modification_validation_final']) < 1) {
    $GLOBALS['_ALERTES']['choisir_sujet_modification_validation_final'] = 1;
}
