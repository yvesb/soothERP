...
class contact_fournisseur extends contact_profil {
	...
	function __construct($ref_fournisseur = "", $action = "open") {
		global $bdd, $DEFAUT_COMPTE_TIERS_ACHAT;
		// Controle si la ref_contact est précisée
		if (!$ref_fournisseur) {return false;}
		$this->ref_fournisseur = $ref_fournisseur;
		if ($action == "create") {return false;}
		$query		 = "SELECT af.ref_fournisseur, af.id_fournisseur_categ, code_client, af.ref_acheteur, af.conditions_commerciales,af.id_stock_livraison, af.delai_livraison, af.app_tarifs, af.defaut_numero_compte,fc.defaut_numero_compte as categ_defaut_numero_compte					
		...
		//remplissage du numéro de compte achat par soit celui de la categorie fournisseur soit par celui par defaut
		if (!$this->defaut_numero_compte) {$this->defaut_numero_compte = $contact_fournisseur->categ_defaut_numero_compte;}
		if (!$this->defaut_numero_compte) {$this->defaut_numero_compte = $DEFAUT_COMPTE_TIERS_ACHAT;}
		$this->ref_contact = $this->ref_fournisseur;$this->profil_loaded = true;
	}

// CREATION DES INFORMATIONS DU PROFIL [FOURNISSEUR]  
	function create_infos($infos) {
		global $DIR, $CONFIG_DIR,$bdd,$DEFAUT_ID_FOURNISSEUR_CATEG,$DEFAUT_ID_STOCK_LIVRAISON,$DEFAUT_APP_TARIFS_FOURNISSEUR;
		// Controle si ces informations sont déjà existantes + Fichier de configuration de ce profil
		if ($this->profil_loaded) {return false;}
		include_once ($CONFIG_DIR . "profil_fournisseur.config.php");
		$this->id_fournisseur_categ = $DEFAUT_ID_FOURNISSEUR_CATEG;
		if (isset($infos['id_fournisseur_categ']) && $infos['id_fournisseur_categ']) {$this->id_fournisseur_categ = $infos['id_fournisseur_categ'];}
		$this->code_client = "";
		if (isset($infos['code_client']) && $infos['code_client']) {$this->code_client = $infos['code_client'];}
		/*
		...
		if (isset($infos['conditions_commerciales'])) {$this->conditions_commerciales = $infos['conditions_commerciales'];}
		$this->id_stock_livraison = $DEFAUT_ID_STOCK_LIVRAISON;
		if (isset($infos['id_stock_livraison']) && is_numeric($infos['id_stock_livraison'])) {$this->id_stock_livraison = $infos['id_stock_livraison'];}
		if (isset($infos['delai_livraison']) && is_numeric($infos['delai_livraison'])) {$this->delai_livraison = $infos['delai_livraison'];}
		$this->app_tarifs = $DEFAUT_APP_TARIFS_FOURNISSEUR;
		if (isset($infos['app_tarifs']) && $infos['app_tarifs']) {$this->app_tarifs = $infos['app_tarifs'];}
		//$this->ref_acheteur 			= $infos['ref_acheteur'];
		...
		if (isset($infos['defaut_numero_compte'])) {
			...
			//on supprime le global alerte que peut générer la cretion du compte pour ne pas bloquer la création du contact
		...
		$bdd->exec($query);return true;
	}

// MODIFICATION DES INFORMATIONS DU PROFIL [FOURNISSEUR]  
	function maj_infos($infos) {
		global $bdd,$DEFAUT_ID_FOURNISSEUR_CATEG,$DEFAUT_ID_STOCK_LIVRAISON;

		if (!$this->profil_loaded) {$GLOBALS['_ALERTES']['profil_non_chargé'] = 1;}
		// Controle des informations
		$this->id_fournisseur_categ = $DEFAUT_ID_FOURNISSEUR_CATEG;
		if (isset($infos['id_fournisseur_categ']) && $infos['id_fournisseur_categ']) {$this->id_fournisseur_categ = $infos['id_fournisseur_categ'];}
		if ($infos['app_tarifs'] != "HT" && $infos['app_tarifs'] != "TTC") {$GLOBALS['_ALERTES']['bad_app_tarifs'] = 1;}
		$this->code_client = "";
		if (isset($infos['code_client']) && $infos['code_client']) {$this->code_client = $infos['code_client'];}
		/*
		  if (isset($infos['ref_acheteur']) && $infos['ref_acheteur']) {
		...
		if (!$this->app_tarifs) {
			...
			$resultat			 = $bdd->query($query);
			if ($annuaire_categories == $resultat->fetchObject()) {$this->app_tarifs = $annuaire_categories->app_tarifs;}
		}
...
// Chargement des derniers documents en cours concernant ce client
	function charger_last_docs($id_type_doc, $is_open = 0) {
		global $bdd, $CONTACT_NB_LAST_DOCS_SHOWED;
		...
		$resultat	 = $bdd->query($query);
		while ($doc		 = $resultat->fetchObject()) {$last_docs[] = $doc;}
		return $last_docs;
	}
...
// FONCTIONS DE LECTURE DES DONNEES 
	function getRef_fournisseur() {return $this->ref_fournisseur;}
	function getId_fournisseur_categ() {return $this->id_fournisseur_categ;}
	function getCode_client() {return $this->code_client;}
	function getRef_acheteur() {return $this->ref_acheteur;}
	function getApp_tarifs() {return $this->app_tarifs;}
	function getDefaut_numero_compte() {return $this->defaut_numero_compte;}
	function getConditions_commerciales() {return $this->conditions_commerciales;}
	function getId_stock_livraison() {return $this->id_stock_livraison;}
	function getDelai_livraison() {return $this->delai_livraison;}
}
