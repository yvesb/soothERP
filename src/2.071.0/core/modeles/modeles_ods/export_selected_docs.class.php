...
class ods_export_selected_docs {
	...
	// Create Ods object
            ...
            $row->addCell( new odsTableCellString("Référence", $bold) );
            $row->addCell( new odsTableCellString("Type de document", $bold) );
            $row->addCell( new odsTableCellString("État", $bold) );
            ...
            foreach($docs as $d) {
                $row   = new odsTableRow();
                $row->addCell( new odsTableCellString($d['ref_doc']) );
                $row->addCell( new odsTableCellString($d['lib_type_doc']) );
                $row->addCell( new odsTableCellString($d['lib_etat_doc']) );
                $row->addCell( new odsTableCellString($d['nom_contact']) );
                ...
                $row->addCell( new odsTableCellString('Le '.date('d/m/Y à H:i:s', strtotime($d['date_doc']))) );
                $this->table->addRow($row);
            }
		...
		// Titre
		$row = new odsTableRow();
		$cell = new odsTableCellString("Liste des documents exportés", $titre);
