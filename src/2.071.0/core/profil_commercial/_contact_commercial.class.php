...
class contact_commercial extends contact_profil {
	...
	function __construct($ref_contact = "", $action = "open") {
		...
		$query		 = "SELECT ac.ref_contact, ac.id_commercial_categ, ac.id_commission_regle,cr.lib_comm, cr.formule_comm, cc.lib_commercial_categ
...
// CREATION DES INFORMATIONS DU PROFIL [commercial]  
	function create_infos($infos) {
		global $DIR, $CONFIG_DIR, $bdd, $DEFAUT_ID_COMMERCIAL_CATEG;
		...
		if (isset($infos['id_commercial_categ']) && $infos['id_commercial_categ']) {$this->id_commercial_categ = $infos['id_commercial_categ'];}
		$this->id_commission_regle = NULL;
		if (isset($infos['id_commission_regle']) && $infos['id_commission_regle']) {$this->id_commission_regle = $infos['id_commission_regle'];}
...
// MODIFICATION DES INFORMATIONS DU PROFIL [commercial]  
	function maj_infos($infos) {
		global $bdd, $DEFAUT_ID_COMMERCIAL_CATEG;
...
// FONCTIONS DE GESTION DES CATEGORIES DE commercial
	static public function charger_commerciaux_categories() {
		...
		$query					 = "SELECT id_commercial_categ , lib_commercial_categ, cc.id_commission_regle,
									 cr.lib_comm, cr.formule_comm, note
						FROM commerciaux_categories cc
	...
	static public function create_commerciaux_categories($infos) {
		global $bdd;
		// Insertion des données
		$query = "INSERT INTO commerciaux_categories (lib_commercial_categ, id_commission_regle, note) 
						VALUES ('" . addslashes($infos['lib_commercial_categ']) . "', " . num_or_null($infos['id_commission_regle']) . "', " . addslashes($infos['note']) . ")";
		$bdd->exec($query);
		return true;
	}
	...
	static public function delete_infos_commerciaux_categories($id_commercial_categ) {
		...
		echo $DEFAUT_ID_COMMERCIAL_CATEG;
...
// FONCTIONS DE LECTURE DES DONNEES 
	function getref_contact() {return $this->ref_contact;}
	function getId_commercial_categ() {return $this->id_commercial_categ;}
	function getId_commission_regle() {return $this->id_commission_regle;}
}
