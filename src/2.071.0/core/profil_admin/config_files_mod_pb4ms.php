...
$new_file_id = fopen ($LIB_DIR_EXT ."phpbackup4mysql/config/tmp_".$file_name, "w");
fwrite($new_file_id, $_REQUEST['new_text_file']);
fclose($new_file_id);

// Remplacement du fichier existant
unlink($LIB_DIR_EXT ."phpbackup4mysql/config/".$file_name);
rename($LIB_DIR_EXT ."phpbackup4mysql/config/tmp_".$file_name, $LIB_DIR_EXT ."phpbackup4mysql/config/".$file_name);
