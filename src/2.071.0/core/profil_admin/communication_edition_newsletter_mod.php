...
$infos										 = array();
$infos['id_newsletter']						 = $_REQUEST["id_newsletter"];
...
$newsletter = new newsletter($infos['id_newsletter']);
$newsletter->maj_newsletter($infos);
$id_newsletter = $newsletter->getId_newsletter();

delete_newsletter_profil($id_newsletter);
foreach ($_REQUEST as $key => $value) {
	if (substr($key, 0, 7) != "profils") {
		continue;
	}
	...
	switch ($value) {
		case $COLLAB_ID_PROFIL:
			...
			foreach ($_REQUEST as $key2 => $value2) {
				if (substr($key2, 0, 16) != "collab_fonction_") {
					continue;
				}
				$collab_fct[] = $value2;
			}
		...
		case $CLIENT_ID_PROFIL:
			...
			$client_cp			 = "";
			foreach ($_REQUEST as $key2 => $value2) {
				if (substr($key2, 0, 17) == "client_categorie_") {
					$client_categorie[] = $value2;
				}
				if (substr($key2, 0, 12) == "client_type_") {
					$client_type[] = $value2;
				}
				if (substr($key2, 0, 13) == "client_categ_") {
					$client_categ[] = $value2;
				}
				if (substr($key2, 0, 9) == "client_cp") {
					$client_cp = $value2;
				}
			}
		...
		case $FOURNISSEUR_ID_PROFIL:
			$fourn_cat = array();
			foreach ($_REQUEST as $key2 => $value2) {
				if (substr($key2, 0, 10) != "fourn_cat_") {
					continue;
				}
				$fourn_cat[] = $value2;
			}
...
// AFFICHAGE
include ($DIR . $_SESSION['theme']->getDir_theme() . "page_communication_edition_newsletter_mod.inc.php");
?>