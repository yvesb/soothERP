...
class pdf_content_doc_fac_lmb_ae extends pdf_content_doc_standard {
	var $code_pdf_modele = "doc_fac_lmb_ae";
...
protected function create_pdf_pieds () {
	...
	foreach($reglements as $reglement) {
		...
		if ($decalage == 3) {
			$this->pdf->SetXY($this->MARGE_GAUCHE, $this->PIEDS_HAUTEUR_DEPART+16+($decalage*4));
			$this->pdf->SetFont('Arial', '', 8);
		...
		$this->pdf->SetXY($this->MARGE_GAUCHE+45, $this->PIEDS_HAUTEUR_DEPART+16+($decalage*4));
		$this->pdf->SetFont('Arial', '', 8);
		$this->pdf->Cell (10, 4, price_format ($reglement->montant_reglement).$MONNAIE[0], '0', 0, 'R');
		
		
		$this->pdf->SetXY($this->MARGE_GAUCHE+55, $this->PIEDS_HAUTEUR_DEPART+16+($decalage*4));
		$this->pdf->SetFont('Arial', '', 8);
