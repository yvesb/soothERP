...
class pdf_content_doc_cdc_lmb_ae extends pdf_content_doc_standard {
	var $code_pdf_modele = "doc_cdc_lmb_ae";
...
protected function create_pdf_pieds () {
	...
	foreach($reglements as $reglement) {
		...
		if ($decalage == 3) {
			$this->pdf->SetXY($this->MARGE_GAUCHE, $this->PIEDS_HAUTEUR_DEPART+($decalage*4));
			$this->pdf->SetFont('Arial', '', 8);
			$this->pdf->Cell ($this->LARGEUR_TOTALE_CORPS-$largeur_bloc_montant, 4, "... ", '0', 0, 'L');
			$decalage ++;
			continue;
		}
		$this->pdf->SetXY($this->MARGE_GAUCHE, $this->PIEDS_HAUTEUR_DEPART+($decalage*4));
		...
		$this->pdf->Cell ($this->LARGEUR_TOTALE_CORPS-$largeur_bloc_montant, 4, "Reglement le ".$date_reglement."   ", '0', 0, 'L');
		
		$this->pdf->SetXY($this->MARGE_GAUCHE+45, $this->PIEDS_HAUTEUR_DEPART+($decalage*4));
		$this->pdf->SetFont('Arial', '', 8);
		$this->pdf->Cell (10, 4, price_format ($reglement->montant_reglement).$MONNAIE[0], '0', 0, 'R');
		
		
		$this->pdf->SetXY($this->MARGE_GAUCHE+55, $this->PIEDS_HAUTEUR_DEPART+($decalage*4));
		$this->pdf->SetFont('Arial', '', 8);
	...
	// Bloc central
	$this->pdf->SetXY($this->MARGE_GAUCHE, $this->PIEDS_HAUTEUR_DEPART+16);
	$this->pdf->SetFont('Arial', '', 7);
	$this->pdf->Cell ($this->LARGEUR_TOTALE_CORPS-$largeur_bloc_montant, 6,"Mention manuscrite « Bon pour accord » + Tampon & Signature" , '0', 0, 'L');
	$this->pdf->SetXY($this->MARGE_GAUCHE, $this->PIEDS_HAUTEUR_DEPART+16);
	$this->pdf->Cell ($this->LARGEUR_TOTALE_CORPS-$largeur_bloc_montant, 16," " , '1', 0, 'L');
