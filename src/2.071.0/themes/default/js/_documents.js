...
//Maj de l'onglet "Article liés" (DIV = link_content) de la pop_up Options
...
function insert_line_nl(indentation_art, indentation_nl) {
    ...
    inputtext.setAttribute("size", "21");
...
//modifier le nombre de ligne de numéro de série en cas de changement de quantité
...
function generer_code_affaire(id_info_content) {
    var AppelAjax = new Ajax.Request(
            ...
            {
                parameters: {code_affaire: encodeURIComponent($F(id_info_content))},
                evalScripts: true,
                onLoading: S_loading, onException: function () {
                    S_failure();
                },
...
/*/maj de l'etat du doc ne demandant qu'un rechargement de l'entete et des commerciaux
 function maj_commerciaux_on_doc () {
 var ref_doc = $("ref_doc").value;
 var AppelAjax = new Ajax.Updater(
 "commercial_name",
 "documents_entete_maj_commerciaux.php",
 {
...
//chargement du contenu des réglements pour un document
...
//	var panier_content = new Array();
    ...
    // calcul du reste due si il existe
    if ($("montant_acquite")) {
...
// ****************************
//fonctions concernant les nombres de lignes et de caractere pour des champ textarea
// ****************************
...
function pre_start_article_line(ref_article, ref_doc_line, indentation) {
    ...
    if ($("link_to_art_" + indentation)) {
        Event.observe("link_to_art_" + indentation, "click", function (evt) {
            Event.stop(evt);
            page.verify('catalogue_articles_view', 'index.php#' + encodeURIComponent('catalogue_articles_view.php?ref_article=' + ref_article), 'true', '_blank');
        }, false);
    }
    //on injecte si il existe, dans les résultats du moteur le numéro d'intendation afin de rendre valide la mise é jour de la qté depuis le moteur
        ...
        if (quantite_locked) {
            ...
            if (tableau.length > 0) {
                ...
                for (compteur = tableau.length - 1; compteur >= 0; compteur--) {
                    ...
                    //Si on est pas remonter jusqu'a  l'id ligne on a trouvé quelque chose
...
//maj d'un numéro de série d'un article à fabriquer
...
//maj d'un numéro de série d'un article à désassembler
...
//double click sur champ sn incrémente les sn suivant si non déjé remplis
...
function is_des_sn_filled() {
    var less_sn = false;
    if (Object.isElement("des_liste_sn") && !$("des_liste_sn").empty()) {
        if ($("qte_des")) {
...
//nouvelle quantitée ajoutée à la derniére ref_doc_line insérée dans le document
...
function check_multiply_ref_doc_line_qte() {
    ...
    if ($("lib_article_r") && last_ssearch_ref_doc_line != "") {
        ...
        if (array_num.length >= 2 && array_num[0] == "*") {
            ...
            //on recherche le champ de quantité correspondant é la ref_doc_line
...
function check_document_compta_lignes() {
    ...
    if (Math.abs(parseFloat($("doc_compta_montant_ttc").value)).toFixed(tarifs_nb_decimales) - Math.abs(parseFloat(total_lines_ttc)).toFixed(tarifs_nb_decimales) <= 0.011) {
...
function check_document_commerciaux_attribution(nombre_lignes) {
    ...
    if (parseInt(total_lines) > 100) {
        //alerte.alerte_erreur ('Erreur dans la répartition', 'La répartition entre les commerciaux doit étre inférieure ou égale é 100%','<input type="submit" id="bouton0" name="bouton0" value="Ok" />');
        //retour = false;
        return retour;
    }
    return retour;
...
/*
 * Fonction permettant de mettre é jour les lignes liées é une ligne (composition, taxes, ...)
 */
...
function doc_modifier_echeance(ref_doc, montant_acquite) {
    var AppelAjax = new Ajax.Updater(
            ...
            {
                parameters: {ref_doc: ref_doc, montant_acquite: montant_acquite},
                evalScripts: true,
                onLoading: S_loading,
                onSuccess: function (requester) {
                    H_loading();
...
function doc_ajouter_echeance(ref_doc, montant_acquite, montant_ech, select_montant_ech, mode_reglement_ech, date_fixe_ech, date_delai_ech, type_ech) {
    var AppelAjax = new Ajax.Request(
            ...
            {
                parameters: {ref_doc: ref_doc, montant_acquite: montant_acquite, montant_ech: montant_ech, select_montant_ech: select_montant_ech, mode_reglement_ech: mode_reglement_ech, date_fixe_ech: date_fixe_ech, date_delai_ech: date_delai_ech, type_ech: type_ech},
                evalScripts: true,
                onLoading: S_loading,
                onSuccess: function (requester) {
                    H_loading();
...
function calcul_modification_duree(indent, ref_article, ref_doc_line, date_deb_abo, duree_mois_abo, duree_jours_abo) {
    if (ref_doc_line != "" && date_deb_abo != "" && duree_mois_abo != "" && duree_jours_abo != "") {
        var AppelAjax = new Ajax.Request(
                ...
                {
                    parameters: {indentation: indent, ref_article: ref_article, ref_doc_line: ref_doc_line, date_deb_abo: date_deb_abo, duree_mois_abo: duree_mois_abo, duree_jours_abo: duree_jours_abo},
                    evalScripts: true,
                    onLoading: S_loading,
                    onSuccess: function (requester) {
                        H_loading();
