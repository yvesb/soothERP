...
//fonction pour ajouter un article(et donc une ligne) à un document de type ticket de caisse
...
function caisse_select_col(col_name) {
	...
	$(selected_col_name + "_" + selected_line_name).removeClassName("ticket_cell_unselected");
	$(selected_col_name + "_" + selected_line_name).addClassName("ticket_cell_selected");
}

function caisse_unselect_col() {
	if (selected_line_name != "" && selected_col_name != "") {
		$(selected_col_name + "_" + selected_line_name).removeClassName("ticket_cell_selected");
		$(selected_col_name + "_" + selected_line_name).addClassName("ticket_cell_unselected");
	}
	selected_col_name = "";
}
...
function caisse_suppr_ticket(ref_ticket, fonction_called_after_maj_etat_ticket) {
	...
	$("bouton_alert_custom").innerHTML = '<table cellpadding="0" cellspacing="0" border="0" width="100%">' +
			...
			'<img id="bouton_pop_up_0" name="bouton_pop_up_0" alt="Valider"  title="Valider"  src="../../themes/default/images/bt-valider.png" />' +
			'</td>' +
			'<td width="10%" >&nbsp;</td>' +
			'<td width="40%" align="left">' +
			'<img id="bouton_pop_up_1" name="bouton_pop_up_1" alt="Annuler" title="Annuler" src="../../themes/default/images/bt-annuler.png" />' +
			'</td>' +
			'</tr>' +
			'</table>';
...
function ajouterArticleTicket(suffixe, monaie, cell_LIB, cell_QTE, cell_PUTTC, cell_REMISE, cell_PRIXTTC) {
	...
	$("TICKET").scrollTop = $("TICKET").offsetHeight;
...
function ajouterInfosTicket(ref_doc_line, titre, text) {
	...
	$("TICKET").insert(tr);
	var td = new Element("td", {"class": "blank"});
	tr.insert(td);

	var td = new Element("td", {'id': 'LIB_' + ref_doc_line, 'class': 'infosT'});
