	...
	<tr class="">
									...
									<table cellpadding="0" cellspacing="0" border="0" style="width:100%;">
										<tr>
											...
											if ($document->getId_etat_doc () == 47) {
												?>
											<img src="<?php echo $DIR.$_SESSION['theme']->getDir_gtheme()?>images/bt_fab_afabriquer.gif" id="fab_afabriquer" style="cursor:pointer"/>
											...
											if ($document->getId_etat_doc () == 49) {
												?>
											<img src="<?php echo $DIR.$_SESSION['theme']->getDir_gtheme()?>images/bt_fab_encours.gif" id="fab_encours" style="cursor:pointer"/>
											<div style="height:3px;line-height:3px;" ></div>
											<?php 
											}
											?>
											<?php 
											if ($document->getId_etat_doc () == 50) {
												?>
											<img src="<?php echo $DIR.$_SESSION['theme']->getDir_gtheme()?>images/bt_fab_terminee.gif" id="fab_terminee" style="cursor:pointer"/>
											<div style="height:3px;line-height:3px;" ></div>
											<?php 
											}
											?>
		...
		Event.observe("date_creation", "blur", function(evt){
			...
			if ($document->getId_etat_doc () == 50) {
				?>
				// pret
				Event.observe("fab_terminee", "click", function(evt){Event.stop(evt);
				 if (is_fab_sn_filled ()) {
					
						$("titre_alert").innerHTML = 'Confirmer';
						$("texte_alert").innerHTML = 'Confirmer la validation du bon de fabrication';
	...
	<tr>
		...
		<table cellpadding="0" cellspacing="0" border="0" style="width:100%" id="document_reglement_entete" class="document_box">
			...
			<tr>
				...
				<div style="OVERFLOW-Y: auto; OVERFLOW-X: auto; width:99%; display:block; height:135px; padding:3px;" id="liste_article_fab">
				<?php
				if ($document->getRef_article() != "" ) {
					$article_to_fab = new article ($document->getRef_article());
					...
					<table width="99%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td style="width:235px">
								<div id="go_view_art_fab" style="cursor:pointer"><?php if ($article_to_fab->getRef_interne()) {echo $article_to_fab->getRef_interne(); } else { echo $document->getRef_article();}?>  <?php echo $article_to_fab->getLib_article();?>
								</div>
								
								<script type="text/javascript">
								Event.observe("go_view_art_fab", "click",  function(evt){Event.stop(evt); page.verify('catalogue_articles_view','index.php#'+escape('catalogue_articles_view.php?ref_article=<?php echo $document->getRef_article()?>'),'true','_blank');}, false);
								</script>
							</td>
						</tr>
						<tr>
							...
							if ($document->getId_etat_doc () != 51) {
								...
								Event.observe('modifier_art_to_fab', "click", function(evt){  
								
									$('id_stock_cata_m').value = $("id_stock").value;
												show_mini_moteur_articles ('set_ref_article_to_fab', '\''+$("ref_doc").value+'\'');
									Event.stop(evt);});
								</SCRIPT>
								<?php
							}
						...
						<tr>
							...
							if ($document->getId_etat_doc () != 51) {
								...
								<SCRIPT type="text/javascript">
								Event.observe('modifier_qte_art_to_fab', "click", function(evt){  
									$("aff_qte_fab").hide();
									$("modifier_qte_art_to_fab").hide();
									$("edi_qte_fab").show();
									$("set_qte_art_to_fab").show();
									Event.stop(evt);
								});
								Event.observe('qte_fab', "blur", function(evt){  
									Event.stop(evt);
									nummask(evt,"0", "X");
								});
								
								Event.observe('set_qte_art_to_fab', "click", function(evt){  
									if ($("raz_doc_1").checked) {
										set_qte_to_fab ("<?php echo $document->getRef_doc();?>", $("qte_fab").value, 1);
									} else {
										set_qte_to_fab ("<?php echo $document->getRef_doc();?>", $("qte_fab").value, 0);
									}
									Event.stop(evt);
								});
								</SCRIPT>
					...
					if ($article_to_fab->getGestion_sn () == 1) {
						$liste_fab_sn = $document->getFab_sn();
						?>
						<div id="fab_liste_sn">
						<?php
						
						
						for ($i=0; $i<$document->getQte_fab(); $i++) {
							?>
							<div id="num_fab_sn_<?php echo $i;?>">
							<span id="more_sn_<?php echo $i;?>" class="more_sn_class" >sn:</span> <input value="<?php if (isset($liste_fab_sn[$i])) {echo $liste_fab_sn[$i];}?>" type="text" id="art_sn_<?php echo $i;?>" name="art_sn_<?php echo $i;?>" /> 
							<input value="<?php if (isset($liste_fab_sn[$i])) {echo $liste_fab_sn[$i];}?>" type="hidden" id="old_art_sn_<?php echo $i;?>" name="old_art_sn_<?php echo $i;?>"/>
							<a href="#" id="sup_sn_<?php echo $i;?>"><img src="<?php echo $DIR.$_SESSION['theme']->getDir_gtheme()?>images/supprime.gif" border="0">		
							</a>
							<script type="text/javascript">
							pre_start_observer_fab_sn ( "<?php echo $i;?>", "<?php echo $document->getRef_doc();?>", "art_sn_<?php echo $i;?>" ,"old_art_sn_<?php echo $i;?>", "sup_sn_<?php echo $i;?>");
							</script>
							</div>
							<?php
						}
						?>
						<input type="hidden" id="art_gest_sn_finliste" name="art_gest_sn_finliste" value="" />
						</div>
						<?php
					}elseif ($article_to_fab->getGestion_sn () == 2) {
						$liste_fab_sn = $document->getFab_nl();
						?>
						<div id="fab_liste_sn">
						<?php
						
						$nb_nl = array();
						foreach ($liste_fab_sn as $diff_nl) {
						...
						foreach ($nb_nl as $nl_key=>$nl_val) {
							...
							<script type="text/javascript">
							pre_start_observer_fab_nl ( "<?php echo $i;?>", "<?php echo $document->getRef_doc();?>", "art_nl_<?php echo $i;?>" ,"old_art_nl_<?php echo $i;?>", "sup_nl_<?php echo $i;?>", "qte_nl_<?php echo $i;?>" ,"old_qte_nl_<?php echo $i;?>", "more_nl_<?php echo $i;?>", "<?php echo $document->getRef_article();?>", "choix_liste_choix_nl_<?php echo $i;?>", "iframe_liste_choix_nl_<?php echo $i;?>" );
							</script>
							</div>
							<?php
							$i++;
						...
						?>
						<input type="hidden" id="art_gest_nl_finliste" name="art_gest_nl_finliste" value="<?php echo $i;?>" />
						</div>
						<img src="<?php echo $DIR.$_SESSION['theme']->getDir_gtheme()?>images/ajouter.gif" width="15px" style="cursor:pointer" id="add_line_nl_content" />
							<script type="text/javascript">
								...
								insert_line_fab_nl ( $("art_gest_nl_finliste").value, "<?php echo $document->getRef_article();?>", "<?php echo $document->getRef_doc();?>");
								},  false );
							</script>
						<?php
					}else{?>
