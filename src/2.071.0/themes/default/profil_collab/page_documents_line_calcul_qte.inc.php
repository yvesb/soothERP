...
foreach ($collisages as $collisage) {
    ?>
                    <option value="<?php echo $collisage; ?>" <?php if ($s) {
                    $s = false;
                    echo 'selected="selected"';
                } ?> ><?php echo $collisage; ?>&nbsp;</option>
<?php } ?>
            </select>
        </td>
        <td ></td>
...
<SCRIPT type="text/javascript">
    ...
    function calculResultat() {
        ...
        if (isNaN(multi)) {
            ...
            if ((multi - multiLite) == 0) {
                $("multiplicateur").value = multiLite;
            }
            else {
            ...
            if (indice_valo > 0 && $("appli_indice_valo").checked) {
                ...
                if (((Math.floor(tmp_res / indice_valo)) * indice_valo) == tmp_res) {
                    ...
                    if ((tmp_res - tmp_resLite) == 0) {
                        $("resultat_calcul").value = tmp_resLite;
                    }
                    else {
                        $("resultat_calcul").value = tmp_res;
                    }
                } else {
