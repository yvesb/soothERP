...
for ($j = 0; $j < count($count_chq_theoriques); $j++) {
    ...
    <?php if (trim($count_chq_theoriques[$j]->infos_supp)) { ?>
                                                                Event.observe("view_info_reg_<?php echo $count_chq_theoriques[$j]->infos_supp; ?>", "click", function (evt) {
                                                                    Event.stop(evt);
...
<?php } ?>
    Event.observe($("chemin_etape_3_2"), "click", function (evt) {
        Event.stop(evt);
        step_menu_controle('controle_validation', 'chemin_etape_3', array_menu_v_controle);
    });
    Event.observe($("chemin_etape_3_3"), "click", function (evt) {
        Event.stop(evt);
        step_menu_controle('controle_validation', 'chemin_etape_3', array_menu_v_controle);
    });

    Event.observe($("bt_etape_3"), "click", function (evt) {
        Event.stop(evt);
        $("controle_caisse_create").submit();
    });
        ...
        switch (key) {
            case Event.KEY_RETURN:
                ...
                for (i = 0; i < array_especes.length; i++) {
                    if (fi_id && $("ESP_" + array_especes[i].replace(".", ""))) {
                        $("ESP_" + array_especes[i].replace(".", "")).focus();
                        break;
                    }
