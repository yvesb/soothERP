									...
									if ($i==$art_categ->indentation) {
										if (key($list_art_categ)!="") {
											if ($art_categ->indentation < current($list_art_categ)->indentation) {
												...
												<img src="<?php echo $DIR.$_SESSION['theme']->getDir_gtheme()?>images/<?php if ($DEFAUT_LEVEL_CATEG_AFFICHED > $i) {echo "extend";} else {echo "collapse";}?>.gif" width="14px" id="collapse_<?php echo $art_categ->ref_art_categ?>" style="display:none"/></a>
												<script type="text/javascript">
...
<div class="articletview_corps" id="liaison_info"  style="OVERFLOW-Y: auto; OVERFLOW-X: auto; width:100%; display:none">
	<div id="liaison_info_under" style="margin-left:5%; margin-right:5%;">
		<input type="text" name="serialisation_liaison" id="serialisation_liaison" style="display:none" value="0" />
		<?php foreach ($liaisons_type_liste as $liaison_type) { 
			...
			<?php foreach ($liaisons_vers_autres_articles as $article_liaison){ ?>
				...
				construire_ligne_liaison_article_duplication(parseInt($("serialisation_liaison").value), "vers", "REF_NOUVEL_ARTICLE", "<?php echo $article_liaison["article_lie"]->getRef_article();?>", <?php echo $liaison_type->getId_liaison_type(); ?>, "<?php echo filtre_js($article_liaison["article_lie"]->getLib_article());?>", <?php echo $article_liaison["ratio"]; ?>);
			<?php } ?>
			</script>
			...
			<?php foreach ($liaisons_depuis_autres_articles as $article_liaison){ ?>
				...
				construire_ligne_liaison_article_duplication(parseInt($("serialisation_liaison").value), "depuis", "REF_NOUVEL_ARTICLE", "<?php echo $article_liaison["article_lie"]->getRef_article();?>", <?php echo $liaison_type->getId_liaison_type(); ?>, "<?php echo filtre_js($article_liaison["article_lie"]->getLib_article());?>", <?php echo $article_liaison["ratio"]; ?>);
			<?php } ?>
			</script>
		<?php } ?>
