...
?>
                    ...
                    <td style=" text-align:right; padding-right:10px; font-size:10px; width:20%; border-right:1px solid #d2d2d2; border-bottom:1px solid #d2d2d2; border-top:1px solid #d2d2d2;">
                        ...
                        <script type="text/javascript">
                            Event.observe("link_compta_reglements_sup_<?php echo $reglement->getRef_reglement(); ?>", "click", function (evt) {
                                Event.stop(evt);
                                alerte.confirm_supprimer('documents_reglements_sup', 'compta_reglements_sup_<?php echo $reglement->getRef_reglement(); ?>');
                            }, false);
                        </script>
                    </td>
                </tr>
            ...
            <form action="reglement_maj.php" method="post" id="reglement_maj" name="reglement_maj" target="formFrame" >
                <input type="hidden" value="<?php echo $reglement->getRef_reglement(); ?>" id="<?php getRef_reglement(); ?>" name="getRef_reglement()" />
...
foreach ($lettrages as $lettrage) {
    ...
    <?php if ($lettrage->montant) {
        echo number_format($lettrage->montant, $TARIFS_NB_DECIMALES, ".", "") . " " . $MONNAIE[1];
    } ?>
                            </td>
