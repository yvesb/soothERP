...
foreach ($choix_sns as $choix_sn) {
	...
	if ($choix_sn->numero_serie != "") {
		?>
		<?php echo  htmlentities($choix_sn->numero_serie, ENT_QUOTES, "UTF-8")?><br />
		<?php 
	}
	?>
	</li>
...
foreach ($choix_sns as $choix_sn) {
	...
	$("<?php echo $_REQUEST["input"]?>").value = "<?php echo  htmlentities($choix_sn->numero_serie, ENT_QUOTES, "UTF-8")?>";
