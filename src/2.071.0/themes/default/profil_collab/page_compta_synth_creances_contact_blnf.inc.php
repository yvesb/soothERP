		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="height:24px;">
			<tr valign="middle">
				...
				<td style=" text-align:left; width:70px; font-weight:bolder;" valign="middle">
					<div style="width:70px; height:24px; line-height:24px; cursor:pointer;" id="order_date_doc_<?php echo$contact->getRef_contact();?>">
						 Date
					</div>
					<script type="text/javascript">
Event.observe("order_date_doc_<?php echo$contact->getRef_contact();?>", "click",  function(evt){
Event.stop(evt); 
page.verify ('compta_livraisons_client_nonfacturees_liste','compta_livraisons_client_nonfacturees_liste.php?orderby=date_creation&orderorder=<?php if (isset($_REQUEST['orderorder']) && $_REQUEST['orderorder']=="ASC" && $_REQUEST['orderby']=="date_creation") {echo "DESC";} else {echo "ASC";}?>&ref_contact=<?php echo$contact->getRef_contact();?>','true','lcnf');
}, false);
					</script>
				</td>
				<td style="width:108px; font-weight:bolder" valign="middle">
					<div style="width:108px; height:24px; line-height:24px; cursor:pointer;" id="order_ref_doc_<?php echo$contact->getRef_contact();?>">
						R&eacute;f&eacute;rence
					</div>
					<script type="text/javascript">
Event.observe("order_ref_doc_<?php echo$contact->getRef_contact();?>", "click",  function(evt){
Event.stop(evt); 
page.verify ('compta_livraisons_client_nonfacturees_liste','compta_livraisons_client_nonfacturees_liste.php?orderby=ref_doc&orderorder=<?php if (isset($_REQUEST['orderorder']) && $_REQUEST['orderorder']=="ASC" && $_REQUEST['orderby']=="ref_doc") {echo "DESC";} else {echo "ASC";}?>&ref_contact=<?php echo$contact->getRef_contact();?>','true','lcnf');
}, false);
					</script>
				</td>
				<td style="font-weight:bolder;text-align:left; font-weight:bolder;" valign="middle">
					<div style=" height:24px; line-height:24px; cursor:pointer;" id="order_nom_contact_<?php echo$contact->getRef_contact();?>">Client
					</div>
					<script type="text/javascript">
Event.observe("order_nom_contact_<?php echo$contact->getRef_contact();?>", "click",  function(evt){
Event.stop(evt); 
page.verify ('compta_livraisons_client_nonfacturees_liste','compta_livraisons_client_nonfacturees_liste.php?orderby=nom_contact&orderorder=<?php if (isset($_REQUEST['orderorder']) && $_REQUEST['orderorder']=="ASC" && $_REQUEST['orderby']=="nom_contact") {echo "DESC";} else {echo "ASC";}?>&ref_contact=<?php echo$contact->getRef_contact();?>','true','lcnf');
}, false);
					</script>
				</td>
				...
				<td style="width:120px;text-align:center; font-weight:bolder;" valign="middle">
					<div style="width:120px; height:24px; line-height:24px; padding-right:20px">
						Stock
					</div>
				</td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="height:24px;">
			<tr valign="middle">
				...
				<td style="width:100px;text-align:right" valign="middle">
					<div style="width:100px; height:24px; line-height:24px; padding-right:20px; font-weight:bolder">
				...
				<td style="width:25px; text-align:right" valign="middle">
					<div style="width:25px; height:24px; line-height:24px;">
						<a href="documents_editing.php?ref_doc=<?php echo $livraison->ref_doc?>&print=1" target="edition" ><img src="<?php echo $DIR.$_SESSION['theme']->getDir_gtheme()?>images/icone_imprime.gif" alt="Imprimer" title="Imprimer"/></a>
					</div>
				</td>
			</tr>
		</table>
	...
	<tr>
		...
		<td style="" >
			<select id="coche_action" name="coche_action" class="classinput_nsize">
				...
				<option value="generer_fa_client">Facturer</option>
...
Event.observe("all_coche", "click", function(evt){
	Event.stop(evt); 
	coche_line ("coche", "<?php echo$contact->getRef_contact();?>" , "commandes")
});
Event.observe("all_decoche", "click", function(evt){
	Event.stop(evt); 
	coche_line ("decoche", "<?php echo$contact->getRef_contact();?>" , "commandes");
});
Event.observe("all_inv_coche", "click", function(evt){
	Event.stop(evt); 
	coche_line ("inv_coche", "<?php echo$contact->getRef_contact();?>" , "commandes");
});

Event.observe("coche_action", "change", function(evt){action_BLC($("coche_action").value, "<?php echo$contact->getRef_contact();?>" , "commandes");});
