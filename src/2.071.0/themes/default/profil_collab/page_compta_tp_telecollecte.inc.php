...
<div class="emarge"><br />
    ...
    <script type="text/javascript">
        Event.observe("link_retour_tp", "click", function (evt) {
            Event.stop(evt);
            page.verify('compta_gestion2_terminaux', 'compta_gestion2_terminaux.php?<?php echo $retour_var; ?>', 'true', 'sub_content');
        }, false);
    </script>
                ...
                <tr>
                    <td rowspan="2" style="width:50px; height:50px; background-color:#FFFFFF">
                        <div style="position:relative; top:-35px; left:-35px; width:105px; border:1px solid #999999; background-color:#FFFFFF; text-align:center">
                            <img src="<?php echo $DIR . $_SESSION['theme']->getDir_gtheme() ?>images/ico_carte.gif" />				</div>
...
for ($j = 0; $j < count($count_cb_theoriques); $j++) {
    ...
    <?php if (trim($count_cb_theoriques[$j]->infos_supp)) { ?>
                                                                Event.observe("view_info_reg_<?php echo $count_cb_theoriques[$j]->infos_supp; ?>", "click", function (evt) {
                                                                    Event.stop(evt);
                                        ...
                                        <td colspan="2">
                                            <div class="emarge"><br />
                                                ...
                                                <div style="text-align:right">
                                                    <img id="bt_etape_2" style=" cursor:pointer; font-weight:bolder; color:#97bf0d; " src="<?php echo $DIR . $_SESSION['theme']->getDir_gtheme() ?>images/bt-valider.gif" />
                                                </div>
                                            </div>
                                            <br />
                                            <br />
...
<SCRIPT type="text/javascript">
    ...
    Event.observe($("chemin_etape_0_2"), "click", function (evt) {
        Event.stop(evt);
        step_menu_telecollecte('telecollecte_date', 'chemin_etape_0', array_menu_v_telecollecte);
    });
    Event.observe($("chemin_etape_0_3"), "click", function (evt) {
        Event.stop(evt);
        step_menu_telecollecte('telecollecte_date', 'chemin_etape_0', array_menu_v_telecollecte);
    });

    Event.observe($("bt_etape_0"), "click", function (evt) {
        Event.stop(evt);
        step_menu_telecollecte('telecollecte_cb', 'chemin_etape_1', array_menu_v_telecollecte);
    });

    Event.observe($("chemin_etape_1_2"), "click", function (evt) {
        Event.stop(evt);
        step_menu_telecollecte('telecollecte_cb', 'chemin_etape_1', array_menu_v_telecollecte);
    });
    Event.observe($("chemin_etape_1_3"), "click", function (evt) {
        Event.stop(evt);
        step_menu_telecollecte('telecollecte_cb', 'chemin_etape_1', array_menu_v_telecollecte);
    });
    ...
    Event.observe($("bt_etape_1"), "click", function (evt) {
        Event.stop(evt);
        step_menu_telecollecte('telecollecte_validation', 'chemin_etape_2', array_menu_v_telecollecte);
    });

    Event.observe($("chemin_etape_2_2"), "click", function (evt) {
        Event.stop(evt);
        step_menu_telecollecte('telecollecte_validation', 'chemin_etape_2', array_menu_v_telecollecte);
    });
    Event.observe($("chemin_etape_2_3"), "click", function (evt) {
        Event.stop(evt);
        step_menu_telecollecte('telecollecte_validation', 'chemin_etape_2', array_menu_v_telecollecte);
    });


    Event.observe($("bt_etape_2"), "click", function (evt) {
        Event.stop(evt);
        $("tp_telecollecte_create").submit();
    });
