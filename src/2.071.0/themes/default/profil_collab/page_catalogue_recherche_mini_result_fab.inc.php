...
<table  cellspacing="0" id="tableresult">
	<tr class="colorise0">
		...
		Quantité à <?php
		if (isset($_REQUEST['fonction_retour']) && ($_REQUEST['fonction_retour'] == "set_ref_article_to_des")) {
			?> désassembler <?php
} else {
			?> fabriquer <?php
		}
...
<tr class="<?php  echo  $class_colorise?>">
	...
	<input type="text" value="0"  id="link_art_lib_qte_to_fab_<?php echo ($fiche->ref_article)?>" name="link_art_lib_qte_to_fab_<?php echo ($fiche->ref_article)?>" size="5" style="text-align:center" />
		<a  href="#" id="val_qte_to_fab_<?php echo ($fiche->ref_article)?>">
			<img src="<?php echo $DIR.$_SESSION['theme']->getDir_gtheme()?>images/bt-ajouter.gif" >
		</a>
			<script type="text/javascript">
			Event.observe("link_art_lib_qte_to_fab_<?php echo ($fiche->ref_article)?>", "blur",  function(evt){
				Event.stop(evt); 
				nummask(evt,"0", "");
			}, false);
			
			Event.observe("val_qte_to_fab_<?php echo ($fiche->ref_article)?>", "click",  function(evt){Event.stop(evt);
			qte_to_send_for_fab = $('link_art_lib_qte_to_fab_<?php echo ($fiche->ref_article)?>').value;
			if ((!isNaN(qte_to_send_for_fab) || qte_to_send_for_fab=="," || qte_to_send_for_fab=="." ) && qte_to_send_for_fab!=" " && qte_to_send_for_fab!="0") {
				<?php echo $_REQUEST['fonction_retour']?>(<?php echo stripslashes($_REQUEST['param_retour'])?>, $('link_art_lib_qte_to_fab_<?php echo ($fiche->ref_article)?>').value, '<?php echo ($fiche->ref_article)?>',"<?php echo addslashes(( str_replace(array("\r\n", "\n", "\r"),"",$fiche->lib_article)))?>", '<?php echo ($fiche->valo_indice)?>');
				}
			}, false);
			</script>
	</td>
