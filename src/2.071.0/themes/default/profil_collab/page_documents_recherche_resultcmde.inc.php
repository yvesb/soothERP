	...
	?>
		...
		?>
			...
			<tr class="colorise1">
				...
				<div id="aff_prix_ht_<?php echo $colorise;?>" style="display:<?php if ($_COOKIE["tarif"] != "HT") { echo "none";} ?>">
					<?php { echo price_format((($article->pu_ht)*($article->qte)), $TARIFS_NB_DECIMALES, ",", ""	)." ".$MONNAIE[1]." HT"; }?>
				</div>
				<div id="aff_prix_ttc_<?php echo $colorise;?>" style="display:<?php if ($_COOKIE["tarif"] != "TTC") { echo "none";} ?>">
		...
		<?php } 
			...
			<tr class="colorise1">
				...
				<td style="width:10%; text-align:center"></td>
				<td style="width:10%; text-align:left; font-weight:bold">Montant total</td>
				<td style="width:10%; text-align:right; font-weight:bold">
				<div id="aff_montant_ht_<?php echo $montant;?>" style="display:<?php if ($_COOKIE["tarif"] != "HT") { echo "none";}?>">
				<?php	 { $montant_total_page_ht +=$fiche->montant_ht; echo price_format($fiche->montant_ht, $TARIFS_NB_DECIMALES, ",", ""	)." ".$MONNAIE[1]." HT";  }?>
				</div>
				<div id="aff_montant_ttc_<?php echo $montant;?>" style="display:<?php if ($_COOKIE["tarif"] != "TTC") { echo "none";}?>">
				...
				<td style="width:15%;  text-align:center">
				<span id="commande_genere_livraison_<?php echo htmlentities($fiche->id_stock."_".$fiche->ref_doc, ENT_QUOTES, "UTF-8");?>" style="color:green; cursor:pointer; display:">[ LIVRAISON >>]<span/>
				<script type="text/javascript">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<td>&nbsp;</td>
		<td>
			Afficher les tarifs 
			<input  name="taxation" id="taxation_ht" type="radio"  value="HT" <?php if ($_COOKIE["tarif"] == "HT") {?> checked="checked"<?php }?>>HT
			<input  name="taxation" id="taxation_ttc" type="radio" value="TTC" <?php if ($_COOKIE["tarif"] == "TTC") {?> checked="checked"<?php }?>>TTC
		</td>
		<td>&nbsp;</td>
...
if ($DOCUMENT_RECHERCHE_MONTANT_TOTAL) {
	...
	<tr class="colorise0">
		...
		<div id="aff_total_ht" style="display:<?php if ($_COOKIE["tarif"] != "HT") { echo "none";}?>">
		<?php echo price_format($montant_total_page_ht, $TARIFS_NB_DECIMALES, ",", ""	)." ".$MONNAIE[1]." HT"?>
		</div>
		<div id="aff_total_ttc" style="display:<?php if ($_COOKIE["tarif"] != "TTC") { echo "none";}?>">
