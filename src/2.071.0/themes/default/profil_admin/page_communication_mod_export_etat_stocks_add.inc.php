...
if (erreur) {
  ...
  if (choisir_id_export_modele) {
    window.parent.document.getElementById("choix_id_export_etat_stocks").className="alerteform_xsize";
    texte_erreur += "Vous devez sélectionner un modèle de document source.<br/>";
  } else {
    window.parent.document.getElementById("choix_id_export_etat_stocks").className="classinput_xsize";
  }
  ...
  if (exist_export_modele) {
    texte_erreur += "Ce modèle d&acute;export est déjà installé dans SoothERP.<br/>";
  }
...
window.parent.page.traitecontent('communication_mod_export_etat_stocks','communication_mod_export_etat_stocks.php','true','sub_content');
