...
<div class="profil_reduce">
    ...
    <div class="reduce_in_edit_mode">
                ...
                <tr>
                    ...
                    <td>
                        <select  id="id_fournisseur_categ"  name="id_fournisseur_categ" class="classinput_xsize">
                            <?php
                            foreach ($liste_categories_fournisseur as
                                        $liste_categorie_fournisseur) {
                                ?>
                                <option value="<?php echo $liste_categorie_fournisseur->id_fournisseur_categ; ?>" <?php
                                if ($profils[$id_profil]->getId_fournisseur_categ() == $liste_categorie_fournisseur->id_fournisseur_categ) {
                                    echo 'selected="selected"';
                                    $id_fournisseur_categ = htmlentities($liste_categorie_fournisseur->id_fournisseur_categ, ENT_QUOTES, "UTF-8");
                                }
                                ?>>
                    ...
                    <td>
                        <select  id="id_stock_livraison"  name="id_stock_livraison" class="classinput_xsize">
                            ...
                            foreach ($stocks_liste as $stock_liste) {
                                ?>
                                <option value="<?php echo $stock_liste->getId_stock(); ?>" <?php
                                if ($stock_liste->getId_stock() == $profils[$id_profil]->getId_stock_livraison()) {
                                    echo 'selected="selected"';
                                    $lib_stock = htmlentities($stock_liste->getLib_stock(), ENT_QUOTES, "UTF-8");
                                }
                                ?>>
                ...
                <tr>
                    <td class="size_strict">
                        ...
                        <select id="app_tarifs" name="app_tarifs" class="classinput_xsize">
                            ...
                            <option value="HT" <?php
                            if ($profils[$id_profil]->getApp_tarifs() == "HT") {
                                echo 'selected="selected"';
                            }
                            ?>>HT</option>
                            <option value="TTC" <?php
                            if ($profils[$id_profil]->getApp_tarifs() == "TTC") {
                                echo 'selected="selected"';
                            }
                            ?>>TTC</option>
                        </select>
        ...
        <script type="text/javascript" language="javascript">
            ...
            Event.observe("show5_delai_livraison", "click", function (evt) {
                Event.stop(evt);
                show_edit_form('annu_edition_profil<?php echo $id_profil ?>', 'start_visible_profil<?php echo $id_profil ?>', 'delai_livraison');
            }, false);

            Event.observe("show5_lib_stock", "click", function (evt) {
                Event.stop(evt);
                show_edit_form('annu_edition_profil<?php echo $id_profil ?>', 'start_visible_profil<?php echo $id_profil ?>', 'conditions_commerciales');
            }, false);

            Event.observe("show5_conditions_commerciales", "click", function (evt) {
                Event.stop(evt);
                show_edit_form('annu_edition_profil<?php echo $id_profil ?>', 'start_visible_profil<?php echo $id_profil ?>', 'conditions_commerciales');
            }, false);

            Event.observe("show5_code_client", "click", function (evt) {
                Event.stop(evt);
                show_edit_form('annu_edition_profil<?php echo $id_profil ?>', 'start_visible_profil<?php echo $id_profil ?>', 'code_client');
            }, false);

            Event.observe("show5_app_tarifs", "click", function (evt) {
                Event.stop(evt);
                show_edit_form('annu_edition_profil<?php echo $id_profil ?>', 'start_visible_profil<?php echo $id_profil ?>', 'app_tarifs');
            }, false);

            Event.observe("show5_id_fournisseur_categ", "click", function (evt) {
                Event.stop(evt);
                show_edit_form('annu_edition_profil<?php echo $id_profil ?>', 'start_visible_profil<?php echo $id_profil ?>', 'id_fournisseur_categ');
            }, false);
