...
foreach ($adresses as $adresse) {
	...
	Event.observe("li_choix_adresse_<?php echo $_REQUEST["input"]?>_<?php echo $i;?>", "click",  function(){$("<?php echo $_REQUEST["input"]?>").value="<?php echo htmlentities($adresse->getRef_adresse (), ENT_QUOTES, "UTF-8")?>"; $("<?php echo $_REQUEST["choix_adresse"]?>").style.display="none"; $("<?php echo $_REQUEST["iframe_choix_adresse"]?>").style.display="none"; $("<?php echo $_REQUEST["div"]?>").innerHTML="<?php if ($adresse->getLib_adresse()!="") {echo addslashes(substr($adresse->getLib_adresse(),0,20));} else { echo addslashes(substr( str_replace (CHR(13), "" ,str_replace (CHR(10), "" ,preg_replace ("#((\r\n)+)#", "", nl2br(htmlentities($adresse->getText_adresse(), ENT_QUOTES, "UTF-8"))))) ."- " . $adresse->getCode_postal() ."- " . $adresse->getVille(),0,20));}?>.."; }, false);
