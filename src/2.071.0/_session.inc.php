...
if (!isset($_SESSION['magasins'])) {
	...
	if (isset($_COOKIE['last_id_magasin'])) {
		$id_magasin = $_COOKIE['last_id_magasin'];
	} else {
		$id_magasin = $DEFAUT_ID_MAGASIN;
	}
...
if (!isset($_SESSION['date_compta_closed'])) {
	...
	if ($compta_e = $resultat->fetchObject()) {
		$_SESSION['date_compta_closed'] = $compta_e->date_fin;
	}
}
...
function page_accessible_a_tous() {
	...
	if (!$_SESSION['user']->getLogin()) {
		if ($_INTERFACE['ID_PROFIL'] == 4 && isset($THIS_DIR)) {
			...
			if ($page_from == $CORE_REP . "site/__user_login.php") {
				$page_from = "";
			}
...
if (isset($_INTERFACE['MUST_BE_LOGIN'])) {
	if (isset($_PAGE['MUST_BE_LOGIN'])) {
		...
		if ($_PAGE['MUST_BE_LOGIN']) {
			page_accessible_a_tous();
		}
	} else {
		//CAS 2
		//$_INTERFACE['MUST_BE_LOGIN'] est défini
		//$_PAGE['MUST_BE_LOGIN'] n'est pas défini
		if ($_INTERFACE['MUST_BE_LOGIN']) {
			page_accessible_a_tous();
		}
	}
} else {
